...
在本教程中 we will build a custom agent that can answer questions about a SQL database using LangGraph.
...
The [prebuilt agent](/oss/python/langchain/sql-agent) lets us get started quickly, but we relied on the system prompt to constrain its behavior—例如， we instructed the agent to always start with the "list tables" tool, and to always run a query-checker tool before executing the query.
