...
1. A **checkpointer** to persist the graph state (use a durable checkpointer 在生产环境中)
...
2. **State is saved** using the checkpointer so execution can be resumed later, 在生产环境中， this should be a persistent checkpointer (e.g. backed by a database)
...
When parallel branches interrupt simultaneously (例如， fan-out to multiple nodes that each call `interrupt()`), you may need to resume multiple interrupts in a single invocation.
When resuming multiple interrupts with a single invocation, map each interrupt ID to its resume value.
This ensures each response is paired with the correct interrupt 在运行时.
...
One of the most common uses of interrupts is to pause before a critical action and ask for approval. 例如， you might want to ask a human to approve an API call, a database change, or any other important decision.
