---
title: 加入和重新加入流
description: 断开并重新连接正在运行的智能体流
---

加入和重新加入功能允许你在不停止智能体的情况下断开正在运行的流，然后稍后重新连接。智能体在客户端离开期间继续在服务器端执行，你可以从中断处精确恢复流。
...
传统的流式 API 将客户端和服务器紧密耦合：如果客户端断开连接，流就丢失了。加入和重新加入打破了这种耦合，支持多种重要场景：
...
| 选项 | 默认值 | 描述 |
|---|---|---|
| `onDisconnect` | `"cancel"` | 客户端断开连接时的行为。`"continue"` 保持智能体运行；`"cancel"` 停止它。 |
| `streamResumable` | `false` | 为 `true` 时，服务器保留流状态以便客户端稍后重新加入。 |

<Tip>
始终同时使用这两个选项。设置 `onDisconnect: "continue"` 但不设置 `streamResumable: true` 意味着智能体继续运行但你无法重新加入流查看输出。
</Tip>
...
<Info>
当运行完成时应清理持久化的运行 ID。监听流完成并移除已存储的 ID，以避免尝试重新加入已完成的运行。
</Info>
