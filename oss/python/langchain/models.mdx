...
<ParamField body="model" type="string" required>
   The name or identifier of the specific model you want to use with a provider. You can also specify both the model and its provider in a single argument using the '{model_provider}:{model}' format, 例如， 'openai:o1'.
</ParamField>
...
LangChain chat models automatically retry failed API requests with exponential backoff. 默认情况下， models retry up to **6 times** for network errors, rate limits (429), and server errors (5xx). Client errors like 401 (unauthorized) or 404 are not retried.
...
<Tip>
    For long-running agent graphs on unreliable networks, consider higher `max_retries` (例如 10–15) and a [checkpointer](/oss/python/langgraph/persistence) so that progress is preserved across failures.
</Tip>

<Info>
    Each chat model integration may have additional params used to control provider-specific functionality.

    例如， [`ChatOpenAI`](https://reference.langchain.com/python/langchain-openai/chat_models/base/ChatOpenAI) has `use_responses_api` to dictate whether to use the OpenAI Responses or Completions API.
...
The resulting message can be treated the same as a message that was generated with [`invoke()`](#invoke)—例如， it can be aggregated into a message history and passed back to the model as conversational context.
...
<Accordion title="Advanced streaming topics">
    ...
    <Accordion title='"Auto-streaming" chat models'>
        ...
        In [LangGraph agents](/oss/python/langchain/agents), 例如， you can call `model.invoke()` within nodes, but LangChain will automatically delegate to streaming if running in a streaming mode.
...
<AccordionGroup>
    ...
    <Accordion title="Forcing tool calls" icon="asterisk">
        默认情况下， the model has the freedom to choose which bound tool to use based on the user's input. However, you might want to force choosing a tool, ensuring the model uses either a particular tool or **any** tool from a given list:
    ...
    <Accordion title="Parallel tool calls" icon="stack-2">
        ...
        <Tip>
        Most models supporting tool calling enable parallel tool calls 默认情况下. Some (including [OpenAI](/oss/python/integrations/chat/openai) and [Anthropic](/oss/python/integrations/chat/anthropic)) allow you to disable this feature. To do this, set `parallel_tool_calls=False`:
        ```python
...
- Controlling resource usage 在生产环境中
