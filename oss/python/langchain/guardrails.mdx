...
安全护栏帮助你构建安全、合规的 AI 应用程序，通过在智能体执行的关键节点验证和过滤内容。它们可以检测敏感信息、执行内容策略、验证输出，并在不安全行为造成问题之前阻止它们。
...
你可以使用[中间件](/oss/python/langchain/middleware)来实现安全护栏，在战略节点拦截执行——在智能体启动前、完成后，或在模型和工具调用周围。
...
LangChain 提供了内置安全护栏（例如 [PII 检测](#pii-detection)、[人机协作](#human-in-the-loop)）以及灵活的中间件系统，用于使用任一方法构建自定义安全护栏。
...
有关 PII 检测功能的完整详情，请参见[中间件文档](/oss/python/langchain/middleware#pii-detection)。
...
<Tip>
    有关实现审批工作流程的完整详情，请参见[人机协作文档](/oss/python/langchain/human-in-the-loop)。
</Tip>
...
- [人机协作](/oss/python/langchain/human-in-the-loop) - 为敏感操作添加人工审查
- [测试智能体](/oss/python/langchain/test/) - 安全机制的测试策略
