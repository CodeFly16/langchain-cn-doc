...
- **Retry-safe:** If the outer middleware implements logic that can result in multiple calls to `handler()` again (例如， retry logic), commands from earlier calls are discarded.
...
Dynamically modify the system prompt 在运行时 to inject context, user-specific instructions, or other information before each model call. This is one of the most common middleware use cases.
...
Select relevant tools 在运行时 to improve performance and accuracy. 本节涵盖 filtering pre-registered tools. For registering tools that are discovered 在运行时 (e.g., from MCP servers), see [Runtime tool registration](/oss/python/langchain/agents#dynamic-tools).
