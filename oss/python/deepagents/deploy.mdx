...
`deepagents deploy` 将你的智能体配置和文件打包在一起，作为 [LangSmith 部署](/langsmith/deployment)进行部署。
...
子智能体允许主智能体将专门的任务委派给隔离的子智能体。每个子智能体有自己的系统提示词、可选的技能和可选的 MCP 工具。主智能体接收一个 `task` 工具，按名称将工作分派给子智能体。
