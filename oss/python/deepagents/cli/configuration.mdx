...
To override model profile fields 在运行时 without editing the config file, pass a JSON object via `--profile-override`:
...
默认情况下， when the CLI loads skills it validates that a resolved skill file path stays inside one of the standard [skill directories](/oss/python/deepagents/data-locations#skills). This prevents symlinks inside skill directories from reading arbitrary files outside those roots.
...
If you switch between terminals with different color schemes (例如， a dark iTerm and a light Apple Terminal), map each one to a theme under `[ui.terminal_themes]`. The CLI matches the shell's `TERM_PROGRAM` and applies the mapped theme automatically:
...
- **No shell 默认情况下**: Commands run with `shell=False`, preventing shell injection.
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/deepagents/cli/configuration.mdx) 或[提交 issue](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>
