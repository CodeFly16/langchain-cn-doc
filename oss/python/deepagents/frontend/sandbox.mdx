...
all threads and may expire), resolve the sandbox per-thread 在运行时. The sandbox reads `thread_id` from the LangGraph config via `getConfig()`:
...
<Info>
  ...
  project files 在运行时 on top of that image.
</Info>
...
- **Use thread-scoped sandboxes** for production apps. Store the sandbox ID in
  thread metadata and resolve it via `getConfig()` 在运行时. This avoids
