...
> 注意：If you change the `model` or any other parameters like `temperature`  or `max_tokens` while setting the client, it will override existing default configurations, that was used in LaunchPad.
...
First we start by defining our repository with some repository ids. 请确保that the ids are valid repository ids. 您可以了解 more about [how to get the repository ID](https://docs.premai.io/get-started/repositories) in the PremAI documentation.
...
This also means that you do not need to make your own RAG pipeline when 使用 Prem Platform. Prem uses it's own RAG technology to deliver best in class performance for Retrieval Augmented Generations.
...
> Please note: As of now, RAG with streaming is not supported. However we still support it with our API. 您可以了解 more in the [chat completion SSE documentation](https://docs.premai.io/get-started/chat-completion-sse).
...
In this section we cover how we can get access to different embedding models using `PremEmbeddings`与 LangChain 配合使用。 Let's start by importing our modules and setting our API Key.
...
- 您可以了解 all about tool calling in details [in our documentation here](https://docs.premai.io/get-started/function-calling).
- 您可以了解 more about langchain tool calling in [this part of the docs](https://python.langchain.com/v0.1/docs/modules/model_io/chat/function_calling).
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/providers/premai.mdx)或[提交 issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
</div>
