...
`ChatNVIDIA` provides chat completions over NVIDIA-hosted models and local NIM deployments. 它支持 tool calling, structured output, image inputs, and streaming.
...
`NVIDIARAGRetriever` connects LangChain to a running [NVIDIA RAG Blueprint](https://docs.nvidia.com/rag/latest/index.html) server and retrieves relevant documents via the `/v1/search` endpoint. 它支持 reranking, query rewriting, and metadata filtering.
...
The `langchain-nvidia-langgraph` package provides NVIDIA-optimized execution strategies for LangGraph graphs. 它提供 two complementary optimizations applied at compile time:
...
The NVIDIA NeMo Agent Toolkit 是一个开源的 AI toolkit for building, profiling, and optimizing agents. Developers can use LangChain with NeMo Agent Toolkit with minimal code changes to enable profiling, evaluation, GPU capacity plans, and automated optimization. NeMo Agent Toolkit is interoperable with LangSmith.
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/providers/nvidia.mdx)或[提交 issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
</div>
