...
目前，there are two Upstash integrations available for LangChain:
...
The Upstash Vector integration in langchain 是一个封装器 for the Upstash Vector Python SDK. That's why the `upstash-vector` package is required.
...
An alternative way of `UpstashVectorStore` is to pass `embedding=True`. 这是一个 unique
...
When inserting documents, first they are embedded 使用 [`Embeddings`](https://reference.langchain.com/python/langchain-core/embeddings/embeddings/Embeddings) object.

Most embedding models can embed multiple documents at once, so the documents are batched and embedded in parallel.
The size of the batch can be controlled 使用 `embedding_chunk_size` parameter.
...
The size of the batch can be controlled 使用 `batch_size` parameter. Upstash Vector has a limit of 1000 vectors per batch in the free tier.
...
您可以获取 information about your database like the distance metric dimension 使用 info function.
...
This page covers how to use [Upstash Redis](https://upstash.com/redis)与 LangChain 配合使用。
...
## Integrations
All of Upstash-LangChain integrations are based on `upstash-redis` Python SDK being utilized as wrappers用于 LangChain。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/providers/upstash.mdx)或[提交 issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
</div>
