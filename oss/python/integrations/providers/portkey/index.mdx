...
- Add `default_headers` to consume the headers needed by Portkey 使用 `createHeaders` helper method.
...
The `virtual_key` parameter sets the authentication and provider for the AI provider being used. In our case we're 使用 Anthropic Virtual key.
...
您可以找到 more [Portkey config examples](https://docs.portkey.ai/docs/api-reference/config-object#examples).
...
**您可以查看 the requests' logs along with the trace id on Portkey dashboard:**
...
您可以查看 out our popular Open Source AI Gateway here - [github.com/portkey-ai/gateway](https://github.com/portkey-ai/gateway)
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/providers/portkey/index.mdx)或[提交 issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
</div>
