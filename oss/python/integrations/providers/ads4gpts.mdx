---
title: "Ads4gpts 集成"
description: "使用 LangChain Python 与 Ads4gpts 集成。"
---

> [ADS4GPTs](https://www.ads4gpts.com/) 正在构建 AI 原生互联网的开放变现基础设施。它以用户体验和隐私优先的方式，帮助 AI 应用通过广告实现变现。

## 安装和设置

### 使用 pip
您可以直接从 PyPI 安装该包：

<CodeGroup>
```bash pip
pip install ads4gpts-langchain
```

```bash uv
uv add ads4gpts-langchain
```
</CodeGroup>

### 从源码安装
或者，从源码安装：

<CodeGroup>
```bash pip
git clone https://github.com/ADS4GPTs/ads4gpts.git
cd ads4gpts/libs/python-sdk/ads4gpts-langchain
pip install .
```

```bash uv
git clone https://github.com/ADS4GPTs/ads4gpts.git
cd ads4gpts/libs/python-sdk/ads4gpts-langchain
uv add .
```
</CodeGroup>

## 前提条件

- Python 3.11+
- ADS4GPTs API Key（[获取 API Key](https://www.ads4gpts.com)）

## 环境变量

设置以下环境变量用于 API 认证：

```bash
export ADS4GPTS_API_KEY='your-ads4gpts-api-key'
```

或者，API 密钥可以在初始化类时直接传入，或存储在 `.env` 文件中。

## 工具

ADS4GPTs 提供两个主要的变现工具：

### Ads4gptsInlineSponsoredResponseTool
此工具获取原生赞助回复，可无缝集成到您的 AI 应用输出中。

```python
from ads4gpts_langchain import Ads4gptsInlineSponsoredResponseTool
```

### Ads4gptsSuggestedPromptTool
生成赞助提示词建议，以增强用户参与度并提供变现机会。

```python
from ads4gpts_langchain import Ads4gptsSuggestedPromptTool
```
### Ads4gptsInlineConversationalTool
提供对话式赞助内容，自然地融入聊天界面和对话中。

```python
from ads4gpts_langchain import Ads4gptsInlineConversationalTool
```

### Ads4gptsInlineBannerTool
提供内联横幅广告，可在 AI 应用的响应中展示。

```python
from ads4gpts_langchain import Ads4gptsInlineBannerTool
```

### Ads4gptsSuggestedBannerTool
生成横幅广告建议，可作为推荐内容呈现给用户。

```python
from ads4gpts_langchain import Ads4gptsSuggestedBannerTool
```

## 工具包

`Ads4gptsToolkit` 组合了这些工具，便于在 LangChain 应用中使用。

```python
from ads4gpts_langchain import Ads4gptsToolkit
```

---

<div className="source-links">
<Callout icon="terminal-2">
    [将这些文档连接](/use-these-docs)到 Claude、VSCode 等工具，通过 MCP 获取实时答案。
</Callout>
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/providers/ads4gpts.mdx)或[提交 issue](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>
