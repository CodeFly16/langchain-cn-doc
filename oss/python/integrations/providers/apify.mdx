...
>[Apify](https://apify.com) 是一个云平台 for web scraping and data extraction,
...
此集成 enables you to run Actors on the `Apify` platform and load their results into LangChain to feed your vector
...
您可以使用 `ApifyActorsTool` to use Apify Actors with agents.
...
您可以使用 `ApifyWrapper` to run Actors on the Apify platform.
...
您也可以使用 our `ApifyDatasetLoader` to get data from an Apify dataset.
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/providers/apify.mdx)或[提交 issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
</div>
