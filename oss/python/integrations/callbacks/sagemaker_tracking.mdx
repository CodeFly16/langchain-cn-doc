...
description: "使用 LangChain Python 集成 SageMaker tracking。"
---
...
* **Scenario 3**: *Agent with Tools (Chain of Thought)* - A case where multiple tools (search and math) are used in addition 提供自然语言接口 LLM.
...
Once the prompts are logged, we can easily load and convert them to Pandas DataFrame 如下.
