...
description: "使用 LangChain Python 集成 Upstash ratelimit。"
---
...
First, you will need to go to [the Upstash Console](https://console.upstash.com/login) and create a redis database ([请参阅我们的 docs](https://upstash.com/docs/redis/overall/getstarted)). After creating a database, you will need to set the environment variables:
...
请注意 we pass the handler to the `invoke` method instead of passing the handler when defining the chain.
...
这是一个示例 with a chain utilizing an LLM:
