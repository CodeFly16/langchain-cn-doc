...
> Using Argilla, everyone can build robust 语言模型s through faster data curation
...
> 请注意 for this scenario we'll be using Google Search API (Serp API) so you will need to both install `google-search-results` as `pip install google-search-results`, and to set the Serp API Key as `os.environ["SERPAPI_API_KEY"] = "..."` (you can find it at [serpapi.com/dashboard](https://serpapi.com/dashboard)), otherwise the example below won't work.
