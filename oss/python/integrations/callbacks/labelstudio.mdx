...
Next, run `label-studio` on the command line to start the local LabelStudio instance at `http://localhost:8080`. 参见 [Label Studio installation guide](https://labelstud.io/guide/install) for more options.
...
请注意 if the project doesn't exist, it will be created with the specified labeling configuration.
