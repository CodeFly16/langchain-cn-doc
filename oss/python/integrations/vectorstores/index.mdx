...
向量存储用于存储[向量嵌入](/oss/python/integrations/embeddings)数据并执行相似度搜索。
...
| 向量存储 | 按 ID 删除 | 过滤 | 按向量搜索 | 带分数搜索 | 异步 | 通过标准测试 | 多租户 | 添加文档时支持 ID |
|------------|-------------|-----------|-----------------|------------------|--------|---------------------|---------------|-------------------|
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/vectorstores/index.mdx)或[提交 issue](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>
