---
title: "Pebblo safe DocumentLoader 集成"
description: "使用 LangChain Python 集成 Pebblo safe DocumentLoader 文档加载器。"
---
...
注意：To configure pebblo server on some URL other that pebblo's default (localhost:8000) URL, put the correct URL in `PEBBLO_CLASSIFIER_URL` env variable. This is configurable using the `classifier_url` keyword argument as well. Ref: [server-configurations](https://daxa-ai.github.io/pebblo/config)
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/document_loaders/pebblo.mdx) 或 [提交 issue](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>
