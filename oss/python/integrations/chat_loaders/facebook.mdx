...
description: "使用 LangChain Python 集成 Facebook messenger。"
---

本 notebook 展示如何从 Facebook 加载数据并转换为可用于微调的格式。 总体步骤如下：

1. 将你的消息数据下载到磁盘。
2. 创建 Chat Loader 并调用 `loader.load()` (or `loader.lazy_load()`) 来执行转换。
3. 可选使用 `merge_chat_runs` 来合并同一发送者的连续消息, and/or `map_ai_messages` 将指定发送者的消息转换为 "AIMessage" 类。 完成后，调用 `convert_messages_for_finetuning` 来准备你的微调数据。

完成后，你可以微调你的模型。为此，你需要完成以下步骤：

4. 将你的消息上传到 OpenAI 并运行微调任务。
