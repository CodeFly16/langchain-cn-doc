---
title: "Imessage 集成"
description: "使用 LangChain Python 集成 Imessage。"
---

本 notebook 展示如何使用 the iMessage chat loader. This class helps convert iMessage conversations to LangChain chat messages.
...
2. Call `loader.load()` (or `loader.lazy_load()`) 来执行转换。 可选使用 `merge_chat_runs` 来合并同一发送者的连续消息, and/or `map_ai_messages` 将指定发送者的消息转换为 "AIMessage" 类。

## 1. Access chat DB

It's likely that your terminal is denied access to `~/Library/Messages`. To use this class, you can copy the DB 提供自然语言接口 accessible directory (e.g., Documents) and load from there. Alternatively (and not recommended), you can grant full disk access for your terminal emulator in System Settings > Security and Privacy > Full Disk Access.
...
Provide the loader with the file path to the zip directory. You can optionally specify the user id that maps 提供自然语言接口 ai message as well as configure whether to merge message runs.
...
It's time to fine-tune the model. 确保你已拥有 `openai` installed
...
You can use the resulting model ID directly the [`ChatOpenAI`](https://reference.langchain.com/python/langchain-openai/chat_models/base/ChatOpenAI) model 类。
