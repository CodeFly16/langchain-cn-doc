...
>关联数据中更快地获取价值。 通过原生图、集成搜索引擎和 JSON 支持，
>使用单一查询语言。 `ArangoDB` 可在本地或云端运行。

本 notebook 展示如何使用 LLMs to provide a 自然语言 interface 提供自然语言接口 [ArangoDB](https://github.com/arangodb/arangodb#readme) database.

## 设置

你可以获取一个本地 `ArangoDB` 实例，通过 [ArangoDB Docker 镜像](https://hub.docker.com/_/arangodb) 运行本地 `ArangoDB` 实例：
