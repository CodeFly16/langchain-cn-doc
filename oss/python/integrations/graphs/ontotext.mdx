...
description: "使用 LangChain Python 集成 Ontotext GraphDB。"
---
...
>本 notebook 展示如何使用 LLMs to provide 自然语言 querying (NLQ to SPARQL, also called `text2sparql`) for `Ontotext GraphDB`.
...
* `query_ontology`: a `CONSTRUCT` query that is executed on the SPARQL endpoint and 返回 KG schema statements. We recommend that you store the ontology in its own named graph, which will make it easier to get only the relevant statements (as the example below). `DESCRIBE` queries are not supported, because `DESCRIBE` 返回 Symmetric Concise Bounded Description (SCBD), i.e. also the incoming class links. In case of large graphs with a million of instances, this is not efficient. Check [github.com/eclipse-rdf4j/rdf4j/issues/4857](https://github.com/eclipse-rdf4j/rdf4j/issues/4857)
