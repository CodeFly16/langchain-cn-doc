---
title: "Amazon neptune with cypher 集成"
description: "使用 LangChain Python 集成 Amazon neptune with cypher。"
---
...
>
>此示例展示 the QA chain that queries the `Neptune` graph database using `openCypher` and returns a human-readable response.
>
...
Now, the QA chain and message history storage can be used to create the new `RunnableWithMessageHistory`. 请注意 we must set `query` as the input key to match the format expected by the base chain.
