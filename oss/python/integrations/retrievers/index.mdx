...
title: "检索器集成"
sidebarTitle: "检索器"
description: "使用 LangChain Python 集成检索器。"
---

[检索器](/oss/python/langchain/retrieval#building-blocks)是一个根据非结构化查询返回文档的接口。
它比向量存储更通用。
检索器不需要能够存储文档，只需要能够返回（或检索）文档。
检索器可以从向量存储创建，但也足够广泛以包含 [Wikipedia search](/oss/python/integrations/retrievers/wikipedia/) and [Amazon Kendra](/oss/python/integrations/retrievers/amazon_kendra_retriever/).
...
以下检索器将在外部索引（例如从互联网数据构建的索引）上进行搜索。
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/integrations/retrievers/index.mdx)或[提交 issue](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>
