...
**请确保 OpenAI 库版本为 1.0.0 或更高；否则，请参阅旧版文档 [OpenAI 适配器（旧版）](/oss/python/integrations/adapters/openai-old/)。**

很多人从 OpenAI 开始入门，但想要探索其他模型。LangChain 与众多模型提供商的集成使这变得很容易。虽然 LangChain 有自己的消息和模型 API，但我们也通过提供适配器来尽可能简化探索其他模型的过程，将 LangChain 模型适配到 OpenAI API。
