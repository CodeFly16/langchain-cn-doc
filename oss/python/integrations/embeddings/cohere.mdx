...
description: "使用 LangChain Python 集成 CohereEmbeddings 向量嵌入模型。"
---

本指南将帮助您开始使用 Cohere 向量嵌入模型 using LangChain. For detailed documentation on `CohereEmbeddings` 功能和配置选项的详细文档，请参阅 [API reference](https://reference.langchain.com/python/langchain-cohere/embeddings/CohereEmbeddings).
...
要访问 Cohere embedding 模型，您需要创建一个 Cohere 账户，获取 API 密钥，并安装 `langchain-cohere` 集成包。

### 凭证

前往 [cohere.com](https://cohere.com) 注册 Cohere 并生成 API 密钥。 Once you’ve done this set the COHERE_API_KEY 环境变量：
...
if not os.getenv("COHERE_API_KEY"):
    os.environ["COHERE_API_KEY"] = getpass.getpass("请输入您的 Cohere API 密钥: ")
```
...
For detailed documentation on `CohereEmbeddings` 功能和配置选项的详细文档，请参阅 [API reference](https://reference.langchain.com/python/langchain-cohere/embeddings/CohereEmbeddings).
