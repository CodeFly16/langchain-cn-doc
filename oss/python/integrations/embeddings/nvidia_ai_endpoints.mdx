...
description: "使用 LangChain Python 集成 NVIDIAEmbeddings 向量嵌入模型。"
---
...
本示例介绍如何使用 LangChain 与 the supported [NVIDIA Retrieval QA Embedding Model](https://build.nvidia.com/nvidia/embed-qa-4) for [retrieval-augmented generation](https://developer.nvidia.com/blog/build-enterprise-retrieval-augmented-generation-apps-with-nvidia-retrieval-qa-embedding-model/) 通过 `NVIDIAEmbeddings` class.

For more information on accessing the 聊天模型 through this API, refer to the [ChatNVIDIA](/oss/python/integrations/chat/nvidia_ai_endpoints) documentation.
