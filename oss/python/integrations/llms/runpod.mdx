---
title: "Runpod 集成"
description: "使用 LangChain Python 集成 Runpod 大语言模型 (LLM)。"
---
...
- **Streaming:** For simulated streaming 通过 `/stream` endpoint, the handler must populate the `"stream"` key in the status response with a list of chunk dictionaries, like `[{"output": "token1"}, {"output": "token2"}]`.
