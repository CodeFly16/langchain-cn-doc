...
description: "使用 LangChain Python 集成 SambanovaCloud 大语言模型 (LLM)。"
---

<Warning>
**您当前所在的页面介绍的是 SambaNovaCloud models as text completion models. We recommend you to use the [chat completion models](/oss/python/langchain/models).**
...
本示例介绍如何使用 LangChain 与 SambaNovaCloud models

## 设置

### 凭证

要访问 ChatSambaNovaCloud models you will need to create a [SambaNovaCloud account](https://cloud.sambanova.ai/), get an API key and set it as the `SAMBANOVA_API_KEY` 环境变量：
...
The 集成位于 `langchain-community` package. We also need  to install the [sseclient-py](https://pypi.org/project/sseclient-py/) package this is required to run streaming predictions
...
Open source models often represent the 先进的 in their respective domains. By using these models, developers can leverage the latest advancements in AI research.
...
有关所有 `SambaNovaCloud` llm 功能和配置的详细文档，请前往 [API reference](https://reference.langchain.com/python/langchain-community/llms/sambanova/SambaNovaCloud)
