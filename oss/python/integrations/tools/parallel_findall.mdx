...
description: "使用 LangChain Python 集成 ParallelFindAllTool。"
---
...
`cancel()` aborts an in-flight run by id. The id is returned to the caller that started the run; if you started it with `工具。ainvoke(...)` in a long-running task, capture `findall_id` from the run before awaiting completion.
...
详细文档, head to the [`ParallelFindAllTool`](https://reference.langchain.com/python/langchain-parallel/findall/ParallelFindAllTool) API reference or the [Parallel FindAll API guides](https://docs.parallel.ai/findall-api/findall-quickstart).
