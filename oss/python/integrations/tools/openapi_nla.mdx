---
title: "Natural language API toolkits 集成"
description: "使用 LangChain Python 集成 Natural language API toolkits。"
---
...
### Next, load the 自然语言 API toolkits
...
Since each NLATool exposes a concisee 自然语言 interface to its wrapped API, the top level conversational agent has an easier job incorporating each endpoint to satisfy a user's request.

**Adding the Spoonacular endpoints.**

1. 前往 [Spoonacular API Console](https://spoonacular.com/food-api/console#Profile) and make a free account.
