...
description: "使用 LangChain Python 集成 SearxNG search。"
---

本 notebook 介绍如何使用 a self hosted `SearxNG` search API to search the web。
...
在此示例中 we will be using the `engines` parameters to query wikipedia
...
在此示例中 we will be looking for scientific paper using the `categories` parameter and limiting the results to a `time_range` (not all engines support the time range option).
...
在此示例中 we query for `large 语言模型s` under the `it` category. We then filter the results that come from github.
