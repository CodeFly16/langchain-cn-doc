...
enables LLMs, which are typically good at generating Pandas code, 提供自然语言接口swer questions about larger
...
以下示例s are borrowed from [the Pandas DataFrames agent notebook](/oss/python/integrations/tools/pandas) with some modifications to highlight key differences.
...
This shows how to initialize the agent using the OPENAI_FUNCTIONS agent type. 请注意 this is an alternative to the above.
...
此示例展示 a slightly more complex use case of passing a Bodo DataFrame to `create_bodo_dataframes_agent`
...
are needed 提供自然语言接口swer the question. 请注意 the DataFrame(s) passed to the agent can also be
...
请注意 while Bodo DataFrames supports most common compute intensive operations in Pandas,
