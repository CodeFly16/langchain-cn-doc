---
title: "JSON toolkit 集成"
description: "使用 LangChain Python 集成 the JSON toolkit。"
---

This notebook showcases an agent interacting with large `JSON/dict` objects.
This is useful when you want 提供自然语言接口swer questions about a JSON blob that's too large to fit in the context window of an LLM. The agent is able to iteratively explore the blob to find what it needs 提供自然语言接口swer the user's question.
...
We will use the JSON agent 提供自然语言接口swer some questions about the API spec.
...
Let's see what individual tools are inside the Jira 工具包。
