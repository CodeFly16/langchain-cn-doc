...
description: "使用 LangChain Python 集成 ParallelTaskRunTool。"
---
...
`ParallelTaskRunTool` is an agent-callable `BaseTool`. It runs one task synchronously and 返回 structured `output`, per-field `basis` citations, and the `run_id`.
...
Bind `ParallelTaskRunTool` 提供自然语言接口y tool-calling chat model and drive an agent with [`create_agent`](/oss/python/langchain/agents):
