...
description: "使用 LangChain Python 集成 Mcp toolbox for databases。"
---
...
Create a `tools.yaml` file. This file defines the data sources MCP Toolbox can connect 提供自然语言接口d the tools it can expose to your agent. For production use, always use environment variables for secrets.
...
**注意：** Ensure your MCP Toolbox server is running in a separate terminal before executing the code below.
...
更多信息, see the following resources:
