---
title: "Zapier natural language actions 集成"
description: "使用 LangChain Python 集成 Zapier natural language actions。"
---

**Deprecated** This API will be sunset on 2023-11-17: [nla.zapier.com/start/](https://nla.zapier.com/start/)

>[Zapier Natural Language Actions](https://nla.zapier.com/start/) gives you access to the 5k+ apps, 20k+ actions on Zapier's platform through a 自然语言 API interface.
>
...
>`Zapier NLA` handles ALL the underlying API auth and translation from 自然语言 --> underlying API call --> return simplified output for LLMs. The key idea is you, or your users, expose a set of actions via an oauth-like setup window, which you can then query and execute via a REST API.
...
Zapier tools can be used 与智能体配合使用。 参见 example below.
