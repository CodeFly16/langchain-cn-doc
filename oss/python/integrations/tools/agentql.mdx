...
- `mode`: `"standard"` uses deep data analysis, while `"fast"` trades some depth of analysis for speed and is adequate for most usecases. [了解更多关于 the modes in this guide.](https://docs.agentql.com/accuracy/standard-mode) **Defaults to `"fast"`.**
...
To instantiate **ExtractWebDataBrowserTool**, you need to connect the 工具与 a browser instance.
...
To instantiate **GetWebElementBrowserTool**, you need to connect the 工具与 a browser instance.
...
- `query`: The AgentQL query to execute. Use AgentQL query if you want to extract precisely structured data. 了解更多关于 [how to write an AgentQL query in the docs](https://docs.agentql.com/agentql-query) or test one out in the [AgentQL Playground](https://dev.agentql.com/playground).
...
**注意：** You must define either a `query` or a `prompt` to use AgentQL.
...
To extract data, first you must navigate to a web page using LangChain's [Playwright](https://python.langchain.com/docs/integrations/tools/playwright/) 工具。
...
You can use AgentQL tools with an AI Agent using the `AgentQLBrowserToolkit` . This toolkit includes `ExtractDataBrowserTool` and `GetWebElementBrowserTool`. 这是一个示例 of agentic browser actions that combine AgentQL's toolkit with the Playwright tools.
...
更多信息 on how to use this integration, please refer to the [git repo](https://github.com/tinyfish-io/agentql-integrations/tree/main/langchain) or the [langchain integration documentation](https://docs.agentql.com/integrations/langchain)
