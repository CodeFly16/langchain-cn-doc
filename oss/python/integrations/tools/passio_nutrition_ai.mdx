...
description: "使用 LangChain Python 集成 Passio nutritionai。"
---
...
请注意 this requires an API key - they have a free tier.
...
Now, we can initialize the agent with the LLM, the prompt, and the tools. The agent is responsible for taking in input and deciding what actions to take. Crucially, the Agent does not execute those actions - that is done by the AgentExecutor (next step). 更多信息 about how to think about these components, 请参阅我们的 [conceptual guide](/oss/python/langchain/agents)
...
Finally, we combine the agent (the brains) with the tools inside the AgentExecutor (which will repeatedly call the agent and execute tools). 更多信息 about how to think about these components, 请参阅我们的 [conceptual guide](/oss/python/langchain/agents)
...
We can now run the agent on a few queries! 请注意 for now, these are all **stateless** queries (it won't remember previous interactions).
