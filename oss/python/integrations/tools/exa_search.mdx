...
First, let's set up the 语言模型. 你需要 provide your OpenAI API key:
...
<Note>
**The `max_characters` parameter for **TextContentsOptions** used to be called `max_length` which is now deprecated. 确保 use `max_characters` instead.**
...
所有 Exa API features and configurations, visit the [Exa API documentation](https://docs.exa.ai/).
