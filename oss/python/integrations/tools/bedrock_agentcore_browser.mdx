...
| [Returns artifact](/oss/python/langchain/tools) | Native async | Supports browser interaction | 定价 |
|:-----------------------------------------------:|:------------:|:----------------------------:|:-------:|
| ✅ | ✅ | ✅ | Pay-per-use (AWS) |
...
| `type_text` | Type text in提供自然语言接口 input field |
...
You need AWS credentials configured with permissions for Bedrock AgentCore Browser. 参见 [Amazon Bedrock AgentCore documentation](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/what-is-bedrock-agentcore.html) for required IAM permissions.
...
```python
# 清理 all browser sessions
await toolkit.cleanup()
```

> **注意：** While `create_browser_toolkit()` is synchronous, the `cleanup()` method is asynchronous and must be awaited.
...
所有 features and configurations, see:
