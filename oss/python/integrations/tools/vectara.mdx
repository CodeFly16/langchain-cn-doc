---
title: "Vectara 集成"
description: "使用 LangChain Python 集成 Vectara。"
---
...
1. Include in your environment these two variables: `VECTARA_API_KEY`.
   ...
   例如, you can set these variables using os.environ and getpass 如下:
...
请注意 we use the `VectaraAddFiles` tool which does not require any local processing or chunking - Vectara receives the file content and performs all the necessary pre-processing, chunking and embedding of the file into its knowledge store.
...
The code below 演示如何 use Vectara tools with LangChain to create an agent.
