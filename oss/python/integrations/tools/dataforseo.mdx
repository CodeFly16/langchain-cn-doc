...
本 notebook 演示如何使用 the [DataForSeo API](https://dataforseo.com/apis) to obtain search engine results.
...
The run method 将返回 the first result snippet from one of the following elements: answer_box, knowledge_graph, featured_snippet, shopping, organic.
...
`run` and `results` are two methods provided by the `DataForSeoAPIWrapper` 类。

The `run` method executes the search and 返回 first result snippet from the answer box, knowledge graph, featured snippet, shopping, or organic results. These elements are sorted by priority from highest to lowest.
...
The API wrapper also allows you to specify the type of search you want to perform. 例如, you can perform a maps search.
