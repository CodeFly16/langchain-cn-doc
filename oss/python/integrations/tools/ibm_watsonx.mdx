...
description: "使用 LangChain Python 集成 IBM watsonx.ai。"
---

>`WatsonxToolkit` is a wrapper for IBM [watsonx.ai](https://www.ibm.com/products/watsonx-ai) Toolkit.

此示例展示 how to use `watsonx.ai` Toolkit using `LangChain`.
...
Initialize the `WatsonxToolkit` 类。
...
For certain requirements, there is an option to pass the IBM's [`APIClient`](https://ibm.github.io/watsonx-ai-python-sdk/base.html#apiclient) object into the `WatsonxToolkit` 类。
...
### Invoke the 工具与 a configuration

To check if a tool has a config schema and view its properties you can look at the tool's `tool_config_schema`.

在此示例中, the tool has a config schema that contains `maxResults` parameter to set maximum number of results to be returned.
...
### Invoke the 工具与 an input schema
...
在此示例中, the tool has an input schema that contains one required and one optional parameter.
...
### Invoke the 工具与 a ToolCall

We can also invoke the 工具与 a ToolCall, in which case a ToolMessage will be returned:
...
所有 `WatsonxToolkit` 功能和配置的详细文档请前往 [API reference](https://reference.langchain.com/python/integrations/langchain_ibm/WatsonxToolkit/).
