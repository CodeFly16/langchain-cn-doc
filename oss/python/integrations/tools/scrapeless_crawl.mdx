...
description: "使用 LangChain Python 集成 Scrapeless crawl。"
---
...
You'll need a Scrapeless API key to use this 工具。 You can set it as an environment variable:
...
- `include_paths` (optional, List[str]): URL pathname regex patterns to include matching URLs in the crawl. Only URLs matching these patterns will be included. 例如, setting `["blog/.*"]` will only include URLs under the `/blog/` path. Default is None.
- `exclude_paths` (optional, List[str]): URL pathname regex patterns to exclude matching URLs from the crawl. 例如, setting `["blog/.*"]` will exclude URLs under the `/blog/` path. Default is None.
...
- `max_discovery_depth` (optional, int): Maximum crawl depth based on discovery order. Root and sitemapped pages have depth `0`. 例如, setting to `1` and ignoring sitemap will crawl only the entered URL and its immediate links. Default is None.
