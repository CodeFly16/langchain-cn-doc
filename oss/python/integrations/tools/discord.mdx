...
This guide provides a quick overview for getting started with Discord tooling in [langchain_discord](/oss/python/integrations/tools/). 更多详情 on each tool and configuration, see the docstrings in your repository or relevant doc pages.
...
The integration is provided by the `langchain-discord-shikenso` package. Install it 如下:
...
下面是一个示例 showing how to instantiate the Discord tools in `langchain_discord`. Adjust as needed for your specific usage.
...
Below is a simple example of calling the 工具与 keyword arguments in a dictionary.
...
If you have a model-generated `ToolCall`, pass it to `工具。invoke()` in the format shown below.
