...
description: "使用 LangChain Python 集成 Google drive。"
---
...
The location of `token.json` use the same directory (or use the parameter `token_path`). 请注意 `token.json` will be created automatically the first time you use the 工具。
...
It's possible to update or customize this. 参见文档 of `GoogleDriveAPIWrapper`.
