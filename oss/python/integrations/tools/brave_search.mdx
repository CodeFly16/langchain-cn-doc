...
本 notebook 介绍如何使用 Brave Search 工具。
前往 [Brave Website](https://brave.com/search/api/) 注册免费账户并获取 API 密钥。
...
# 或者如果你想从环境变量获取 API 密钥 BRAVE_SEARCH_API_KEY，search_kwargs 留空
# tool = BraveSearch()

# 或者如果你只想提供 API 密钥，search_kwargs 留空
# tool = BraveSearch.from_api_key(api_key=api_key)

# 或者如果你只想提供 search_kwargs 并从 BRAVE_SEARCH_API_KEY 环境变量读取 API 密钥
