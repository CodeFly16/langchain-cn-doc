...
本指南提供了开始使用 `ChatReka` [聊天模型](/oss/python/langchain/models).

Reka has several 聊天模型. You can find information about their latest models and their costs, context windows, and supported input types in the [Reka docs](https://docs.reka.ai/available-models).
...
要访问 Reka 模型，您需要创建一个 Reka developer 账户，获取 API 密钥，并安装 `langchain_community` integration package and the reka python package via 'pip install reka-api'.

### 凭证

前往 [platform.reka.ai/](https://platform.reka.ai/) 注册 Reka 并生成 API 密钥。 完成后设置 REKA_API_KEY 环境变量：

### 安装

LangChain 的 ChatReka 集成位于 `langchain_community` 包中：
...
We can 查看 the LangSmith trace to make sure it's calling the search tool effectively.
