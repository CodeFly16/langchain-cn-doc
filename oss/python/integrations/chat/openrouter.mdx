...
本指南将帮助您开始使用 OpenRouter [聊天模型](/oss/python/langchain/models). OpenRouter is a unified API that provides access to models from multiple providers (OpenAI, Anthropic, Google, Meta, and more) through a single endpoint.

<Tip>
    **API 参考**

    有关所有 features and configuration options, 请前往 [`ChatOpenRouter`](https://reference.langchain.com/python/integrations/langchain_openrouter) API reference.
</Tip>
...
| :--- | :--- | :---: |  :---: | :---: | :---: |
| [`ChatOpenRouter`](https://reference.langchain.com/python/integrations/langchain_openrouter) | [`langchain-openrouter`](https://reference.langchain.com/python/integrations/langchain_openrouter) | beta | ❌ | <a href="https://pypi.org/project/langchain-openrouter/" target="_blank"><img src="https://static.pepy.tech/badge/langchain-openrouter/month" alt="下载量 per month" noZoom height="100" class="rounded" /></a> | <a href="https://pypi.org/project/langchain-openrouter/" target="_blank"><img src="https://img.shields.io/pypi/v/langchain-openrouter?style=flat-square&label=%20&color=orange" alt="PyPI - Latest version" noZoom height="100" class="rounded" /></a> |
...
要访问 models via OpenRouter you'll need to create an [OpenRouter account](https://openrouter.ai/), get an API key, and install the `langchain-openrouter` 集成包。
...
Head to the [OpenRouter keys page](https://openrouter.ai/settings/keys) to sign up 并生成 API 密钥。 完成后设置 `OPENROUTER_API_KEY` 环境变量：
...
`ChatOpenRouter` supports structured output 通过 [`with_structured_output`](/oss/python/langchain/models#structured-output) method. Two methods are available: `function_calling` (default) and `json_schema`.
...
For models that support reasoning (e.g., `anthropic/claude-sonnet-4.5`, `deepseek/deepseek-r1`), you can enable reasoning tokens 通过 `reasoning` parameter. 请参阅 [OpenRouter reasoning docs](https://openrouter.ai/docs/guides/best-practices/reasoning-tokens) for details:
...
For more on content blocks, 请参阅 [standard content blocks](/oss/python/langchain/messages#standard-content-blocks) guide.
...
<Note>
    The effort-to-budget mapping is model-dependent. For example, Google Gemini models map effort to an internal `thinkingLevel` rather than an exact token budget. 请参阅 [OpenRouter reasoning docs](https://openrouter.ai/docs/guides/best-practices/reasoning-tokens) for details.
</Note>
...
请参阅 [OpenRouter provider routing docs](https://openrouter.ai/docs/guides/routing/provider-selection) for the full list of options.
...
OpenRouter can broadcast request data to configured observability destinations. `ChatOpenRouter` exposes two related parameters: `session_id` for grouping related requests under a single logical workflow, and `trace` for per-request trace metadata. 请参阅 [OpenRouter broadcast docs](https://openrouter.ai/docs/guides/features/broadcast/overview) for details.
...
有关所有 `ChatOpenRouter` 功能和配置的详细文档，请前往 [`ChatOpenRouter`](https://reference.langchain.com/python/integrations/langchain_openrouter) API reference.

For more information about OpenRouter's platform, models, and features, 请参阅 [OpenRouter documentation](https://openrouter.ai/docs).
