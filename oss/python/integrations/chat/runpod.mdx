...
Get started with RunPod 聊天模型.

## 概述

This guide covers how to use the LangChain `ChatRunPod` class to interact with 聊天模型 hosted on [RunPod Serverless](https://www.runpod.io/serverless-gpu).
...
# 可选ly use a different endpoint ID specifically for 聊天模型
...
**Key Takeaway:** Standard chat invocation and simulated streaming work if the endpoint follows basic RunPod API conventions. Advanced features require specific handler implementations and potentially extending or customizing this 集成包。
