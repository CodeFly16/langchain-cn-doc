...
| 模型 | Latency | Web browsing | Citations | 结构化输出 | Use when |
| :--- | :--- | :---: | :---: | :---: | :--- |
...
要访问 Parallel models, install the `langchain-parallel` integration package and acquire a [Parallel](https://platform.parallel.ai) API key.
...
前往 [Parallel](https://platform.parallel.ai) to sign up 并生成 API 密钥。 Set `PARALLEL_API_KEY` in your environment:
...
请参阅 [`ChatParallel`](https://reference.langchain.com/python/langchain-parallel/chat_models/ChatParallel) API reference for the full set of available parameters.
...
For detailed documentation, 请前往 [`ChatParallel`](https://reference.langchain.com/python/langchain-parallel/chat_models/ChatParallel) API reference or the [Parallel chat API quickstart](https://docs.parallel.ai/chat-api/chat-quickstart).
