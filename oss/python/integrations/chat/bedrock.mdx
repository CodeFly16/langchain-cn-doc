...
For Anthropic models specifically, you can use `ChatAnthropicBedrock` which extends `ChatAnthropic` and provides the same API while running on AWS Bedrock. 请参阅 [ChatAnthropicBedrock section](#chatanthropicbedrock) below for details.
</Info>

有关所有 Bedrock 功能和配置的详细文档，请前往 [API reference](https://reference.langchain.com/python/langchain-aws/chat_models/bedrock_converse/ChatBedrockConverse).
...
Head to the [AWS docs](https://docs.aws.amazon.com/bedrock/latest/userguide/setting-up.html) 注册 AWS and setup your credentials.
...
当使用 [tool calling](/oss/python/langchain/tools) or [structured output](/oss/python/langchain/structured-output) with Anthropic models, tool call arguments stream as partial JSON chunks by default.
...
<Note>
    Fine-grained tool streaming is supported on Claude 4.5+ models. 请参阅 [Claude documentation](https://platform.claude.com/docs/en/agents-and-tools/tool-use/fine-grained-tool-streaming) 了解更多详情。
</Note>
...
有关所有 `ChatBedrock`, `ChatBedrockConverse`, and `ChatAnthropicBedrock` features and configurations head to [the API reference](https://reference.langchain.com/python/langchain-aws).
