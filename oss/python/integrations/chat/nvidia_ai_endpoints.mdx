---
title: "ChatNVIDIA 集成"
description: "使用 LangChain Python 集成 ChatNVIDIA and ChatNVIDIADynamo 聊天模型 。"
---

本指南将帮助您开始使用 NVIDIA [聊天模型](/oss/python/langchain/models). 有关所有 `ChatNVIDIA` 功能和配置的详细文档，请前往 [API reference](https://reference.langchain.com/python/langchain-nvidia-ai-endpoints/chat_models/ChatNVIDIA).

## 概述

The `langchain-nvidia-ai-endpoints` package contains LangChain integrations for 聊天模型 and embeddings powered by [NVIDIA AI Foundation Models](https://www.nvidia.com/en-us/ai-data-science/foundation-models/), and hosted on the [NVIDIA API Catalog](https://build.nvidia.com/).
...
For more information on accessing 向量嵌入模型 through this API, refer to the [`NVIDIAEmbeddings`](/oss/python/integrations/embeddings/nvidia_ai_endpoints) documentation.
...
These models accept the same arguments and input structure as regular 聊天模型, but they tend to perform better on code-generation and structured code tasks. An example of this is `meta/codellama-70b`.
...
See [How to use 聊天模型 to call tools](/oss/python/langchain/tools) for additional examples.
