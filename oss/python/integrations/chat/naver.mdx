...
本指南提供了通过 CLOVA Studio 使用 Naver HyperCLOVA X [聊天模型](https://python.langchain.com/docs/concepts/chat_models) 的快速入门概览。有关 `ChatClovaX` 所有功能和配置的详细文档，请前往 [API 参考](https://guide.ncloud-docs.com/docs/clovastudio-dev-langchain)。
...
3. 创建要使用的模型的 CLOVA Studio 测试应用或服务应用（参见 [CLOVA Studio 测试应用设置](https://guide.ncloud-docs.com/docs/clovastudio-playground-testapp)。）
4. 发放测试或服务 API 密钥（参见 [Naver API 密钥文档](https://api.ncloud-docs.com/docs/ai-naver-clovastudio-summary#API%ED%82%A4)。）

### 凭证

设置 `CLOVASTUDIO_API_KEY` 环境变量为您的 API 密钥。
...
除了下面的 `invoke` 之外，`ChatClovaX` 还支持批处理、流式输出及其异步功能。
...
CLOVA Studio 支持工具调用（也称为"[函数调用](https://api.ncloud-docs.com/docs/clovastudio-chatcompletionsv3-fc)"），允许您描述工具及其参数，并让模型返回一个包含要调用的工具和该工具输入的 JSON 对象。这对于构建使用工具的链和智能体非常有用，也更广泛地用于从模型获取结构化输出。

**注意**：您应该将 `max_tokens` 设置为大于 1024 才能在 CLOVA Studio 中使用工具调用功能。

### ChatClovaX.bind_tools()

通过 `ChatClovaX.bind_tools`，我们可以轻松地将 Pydantic 类、字典模式、LangChain 工具甚至函数作为工具传递给模型。底层会将这些转换为 OpenAI 兼容的工具模式，如下所示：
...
chat = ChatClovaX(
    model="HCX-005",
    max_tokens=1024,  # 将 max tokens 设置为大于 1024 以使用工具调用
)
```
...
注意 `AIMessage` 具有 [`tool_calls`](https://reference.langchain.com/python/langchain/messages/#langchain.messages.AIMessage.tool_calls) 属性。它包含与模型提供商无关的标准化 `ToolCall` 格式。
...
对于支持的模型，您可以使用[结构化输出](https://api.ncloud-docs.com/docs/clovastudio-chatcompletionsv3-so)功能强制模型以特定结构生成响应，例如 Pydantic 模型、TypedDict 或 JSON。
...
对于支持的模型，当[思考](https://api.ncloud-docs.com/docs/clovastudio-chatcompletionsv3-thinking)功能启用时（默认启用），它将输出得出最终答案的逐步推理过程。
...
当思考模式启用时，您可以通过 `AIMessage.additional_kwargs` 中的 `thinking_content` 属性访问思考过程。
...
fine_tuned_model = ChatClovaX(
    model="ft:a1b2c3d4",  # 设置为 `ft:{task_id}`，使用您微调模型的 task id
    # 其他参数...
)
...
有关 `ChatClovaX` 所有功能和配置的详细文档，请前往 [API 参考](https://guide.ncloud-docs.com/docs/clovastudio-dev-langchain)
