...
<Tip>
    **API 参考**
    ...
    有关所有 features and configuration options, 请前往 [`ChatAnthropic`](https://reference.langchain.com/python/langchain-anthropic/chat_models/ChatAnthropic) API reference.
</Tip>

<Info>
    **AWS Bedrock and Google VertexAI**

    Note that certain Anthropic models can also be accessed via AWS Bedrock and Google VertexAI. 请参阅 [`ChatBedrock`](/oss/python/integrations/chat/bedrock/) and [`ChatVertexAI`](/oss/python/integrations/chat/google_vertex_ai) integrations to use Anthropic models 通过se services.
...
| 类 | 包 | 可序列化 | JS/TS Support | 下载量 | Latest 版本 |
| :--- | :--- | :---: |  :---: | :---: | :---: |
| [`ChatAnthropic`](https://reference.langchain.com/python/langchain-anthropic/chat_models/ChatAnthropic) | [`langchain-anthropic`](https://reference.langchain.com/python/langchain-anthropic/) | beta | ✅ [(npm)](https://js.langchain.com/docs/integrations/chat/anthropic) | <a href="https://pypi.org/project/langchain-anthropic/" target="_blank"><img src="https://static.pepy.tech/badge/langchain-anthropic/month" alt="下载量 per month" noZoom height="100" class="rounded" /></a> | <a href="https://pypi.org/project/langchain-anthropic/" target="_blank"><img src="https://img.shields.io/pypi/v/langchain-anthropic?style=flat-square&label=%20&color=orange" alt="PyPI - Latest version" noZoom height="100" class="rounded" /></a> |
...
要访问 Anthropic (Claude) models you'll need to install the `langchain-anthropic` integration package and acquire a [Claude](https://platform.claude.com/docs/en/get-started#prerequisites) API key.
...
Head to the [Claude console](https://console.anthropic.com) to sign up and generate a Claude API key. 完成后设置 `ANTHROPIC_API_KEY` 环境变量：
...
请参阅 [`ChatAnthropic`](https://reference.langchain.com/python/langchain-anthropic/chat_models/ChatAnthropic) API reference for details on all available instantiation parameters.
...
当使用 tools, [extended thinking](#extended-thinking), and other features, content from a single Anthropic [`AIMessage`](https://reference.langchain.com/python/langchain-core/messages/ai/AIMessage) can either be a single string or a list of Anthropic content blocks.
...
<Note>
    For information about Claude's built-in tools (code execution, web browsing, files API, etc), 请参阅 [Built-in tools](#built-in-tools).
</Note>
...
<Info>
    Strict tool use requires `langchain-anthropic>=1.1.0`. 请参阅 [Claude docs](https://platform.claude.com/docs/en/agents-and-tools/tool-use/strict-tool-use) for supported models.
</Info>
...
Strict tool use has some JSON schema limitations to be aware of. 请参阅 [Claude docs](https://platform.claude.com/docs/en/build-with-claude/structured-outputs#json-schema-limitations) 了解更多详情。
...
<Info>
    Programmatic tool calling requires `langchain-anthropic>=1.3.0`. 请参阅 [Claude docs](https://platform.claude.com/docs/en/agents-and-tools/tool-use/programmatic-tool-calling) for supported models.
</Info>
...
<Tip>
    The Files API can also be used to upload files to a container for use with Claude's built-in code-execution tools. 请参阅 [code execution](#code-execution) section for details.
</Tip>
...
请参阅 [Claude documentation](https://platform.claude.com/docs/en/build-with-claude/effort) for detail on when to use different effort levels and to see supported models.
...
<Warning>
    Only certain Claude models support prompt caching. 请参阅 [Claude documentation](https://platform.claude.com/docs/en/build-with-claude/prompt-caching#supported-models) for details.
</Warning>
...
description = (
    "Get the weather at a location. "
    f"By the way, 查看 this readme: {readme}"
)
...
请参阅 [Claude documentation](https://platform.claude.com/docs/en/build-with-claude/context-editing) for more details and configuration options.
...
<Info>
    Structured output requires `langchain-anthropic>=1.1.0`. 请参阅 [Claude docs](https://platform.claude.com/docs/en/build-with-claude/structured-outputs) for supported models.
</Info>
...
<Note>
    **Important: You must provide the execution environment**
    ...
    请参阅 [Claude bash tool docs](https://platform.claude.com/docs/en/agents-and-tools/tool-use/bash-tool) for implementation guidance.
</Note>
...
<Note>
    The code sandbox does not have internet access, thus you may only use packages that are pre-installed in the environment. 请参阅 [Claude docs](https://platform.claude.com/docs/en/agents-and-tools/tool-use/code-execution-tool#networking-and-security) for more info.
</Note>
...
Claude supports a client-side text editor tool can be used to view and modify text local files. 请参阅 [text editor tool documentation](https://platform.claude.com/docs/en/agents-and-tools/tool-use/text-editor-tool) for details.
...
Claude supports a memory tool for client-side storage and retrieval of context across conversational threads. 请参阅 [memory tool documentation](https://platform.claude.com/docs/en/agents-and-tools/tool-use/memory-tool) for details.
...
请参阅 [Claude documentation](https://platform.claude.com/docs/en/agents-and-tools/tool-use/tool-search-tool) for more details on tool search, including usage with MCP servers and client-side implementations.
