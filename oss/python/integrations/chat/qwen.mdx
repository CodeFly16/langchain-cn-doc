...
本指南将帮助您开始使用 Qwen [聊天模型](/oss/python/langchain/models). 有关所有 `ChatQwen` 功能和配置的详细文档，请前往 [API reference](https://pypi.org/project/langchain-qwq/).
...
要访问 Qwen 模型，您需要创建一个 Alibaba Cloud 账户，获取 API 密钥，并安装 `langchain-qwq` 集成包。

### 凭证

前往 [Alibaba's API Key page](https://account.alibabacloud.com/login/login.htm?oauth_callback=https%3A%2F%2Fbailian.console.alibabacloud.com%2F%3FapiKey%3D1&lang=en#/api-key) 注册 Alibaba Cloud 并生成 API 密钥。 完成后设置 `DASHSCOPE_API_KEY` 环境变量：
...
有关所有 `ChatQwen` 功能和配置的详细文档，请前往 [API reference](https://pypi.org/project/langchain-qwq/)
