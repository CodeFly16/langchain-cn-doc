...
本指南提供了开始使用 Databricks [聊天模型](/oss/python/langchain/models).
...
1. Foundation Models - Curated list of 先进的 foundation models such as DRBX, Llama3, Mixtral-8x7B, and etc. These endpoint are ready to use in your Databricks workspace without any set up.
...
LangChain 的 Databricks 集成位于 `databricks-langchain` package.
...
Similar to other 聊天模型, `ChatDatabricks` can be used as a part of a complex chain.
...
有关所有 `ChatDatabricks` 功能和配置的详细文档，请前往 API reference: [api-docs.databricks.com/python/databricks-ai-bridge/latest/databricks_langchain.html#databricks_langchain.ChatDatabricks](https://api-docs.databricks.com/python/databricks-ai-bridge/latest/databricks_langchain.html#databricks_langchain.ChatDatabricks)
