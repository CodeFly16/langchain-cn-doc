...
本指南提供了开始使用 `ChatBaseten` [聊天模型](/oss/python/langchain/models) 的快速概览。

Baseten 提供专为生产应用设计的推理服务。基于 Baseten 推理技术栈构建，这些 API 为领先的开源或自定义模型提供企业级性能和可靠性：https://www.baseten.co/library/。
...
模型 API 仅支持文本输入，部分专用部署根据模型支持图像和音频输入。详情请查看 Baseten 模型库：https://www.baseten.co/library/
...
if "BASETEN_API_KEY" not in os.environ:
    os.environ["BASETEN_API_KEY"] = getpass.getpass("请输入您的 Baseten API 密钥: ")
```
...
model = ChatBaseten(
    model="moonshotai/Kimi-K2-Instruct-0905",  # 从可用模型标识符中选择：https://docs.baseten.co/development/model-apis/overview#supported-models
