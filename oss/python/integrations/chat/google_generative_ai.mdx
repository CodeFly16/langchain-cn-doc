...
Access Google's Generative AI models, including the Gemini family, 通过 **Gemini Developer API** or **Vertex AI**. The Gemini Developer API offers quick setup with API keys, ideal for individual developers. Vertex AI provides enterprise features and integrates with Google Cloud Platform.

有关最新模型的信息, model IDs, their features, context windows, etc. 请前往 [Google AI docs](https://ai.google.dev/gemini-api/docs).
...
<Note>
    **Vertex AI consolidation & compatibility**
    ...
    This migration brings support for Gemini models both 通过 Gemini Developer API and Gemini API in Vertex AI, superseding certain classes in `langchain-google-vertexai`, such as `ChatVertexAI`.
...
<Tip>
    **API 参考**
    ...
    有关所有 features and configuration options, 请前往 [`ChatGoogleGenerativeAI`](https://reference.langchain.com/python/langchain-google-genai/chat_models/ChatGoogleGenerativeAI) API reference.
</Tip>
...
要访问 Google AI 模型，您需要创建一个 Google Account, get a Google AI API key, and install the `langchain-google-genai` 集成包。
...
<Tabs>
    <Tab title="Gemini Developer API">
        **Quick setup with API key**
        ...
        前往 [Google AI Studio](https://ai.google.dev/gemini-api/docs/api-key) to generate an API key:
        ...
        if "GOOGLE_API_KEY" not in os.environ:
            os.environ["GOOGLE_API_KEY"] = getpass.getpass("请输入您的 Google AI API 密钥: ")
        ```
    ...
    <Tab title="Vertex AI with credentials">
        **Vertex AI using service account or ADC**
        ...
        ```bash
        export GOOGLE_CLOUD_PROJECT='your-project-id'
        # 可选: set region (defaults to us-central1)
...
<Tabs>
    ...
    <Tab title="Vertex AI">
        ...
        model = ChatGoogleGenerativeAI(
            model="gemini-3.1-pro-preview",
            project="your-project-id", # [!code highlight]
            location="us-central1",  # 可选, defaults to us-central1 [!code highlight]
            temperature=1.0,  # Gemini 3.0+ defaults to 1.0
            max_tokens=None,
            timeout=None,
            max_retries=2,
...
请参阅 [`ChatGoogleGenerativeAI`](https://reference.langchain.com/python/langchain-google-genai/chat_models/ChatGoogleGenerativeAI) API Reference for the full set of available model parameters.
...
export SSL_CERT_FILE='path/to/cert.pem'  # 可选: custom SSL certificate
```
...
<Warning>
    **Vertex AI Limitation**
    ...
    更多详情, see this [Google AI forum discussion](https://discuss.ai.google.dev/t/request-allowlist-access-for-audio-output-in-gemini-2-5-pro-flash-tts-vertex-ai/108067).
</Warning>
...
Force the model to respond with a specific structure. 请参阅 [Gemini API docs](https://ai.google.dev/gemini-api/docs/structured-output) for more info.
...
当使用 `with_structured_output(method="function_calling")`, do not pass additional tools (like Google Search) in the same call.
...
```python
Prompt engineering is the art and science of crafting effective text prompts to elicit desired and accurate responses from 大语言模型.
...
<Warning>
    Not all models allow disabling thinking. 请参阅 [Gemini models documentation](https://ai.google.dev/gemini-api/docs/models) for details.
</Warning>
...
请参阅 [Gemini API docs](https://ai.google.dev/gemini-api/docs/thinking) for more information on thinking.
...
<Warning>
    **Preview model limitations**
    ...
    The Computer Use model is in preview and may produce unexpected behavior. Always supervise automated tasks and avoid use with sensitive data or critical operations. 请参阅 [Gemini API docs](https://ai.google.dev/gemini-api/docs/computer-use) for safety best practices.
</Warning>
...
Context caching allows you to store and reuse content (e.g., PDFs, images) for faster processing. The `cached_content` parameter accepts a cache name created 通过 Google Generative AI API.
