...
<Tip>
    **API 参考**
    ...
    有关所有 features and configuration options, 请前往 [`ChatOpenAI`](https://reference.langchain.com/python/langchain-openai/chat_models/base/ChatOpenAI) API reference.
</Tip>

<Warning>
    **API scope**

    `ChatOpenAI` 仅针对 [OpenAI 官方 API 规范](https://github.com/openai/openai-openapi) 。来自第三方提供商的非标准响应字段 (e.g., `reasoning_content`, `reasoning`, `reasoning_details`) **不会被提取或保留**。 If you are using a provider that extends the Chat Completions or Responses formats, such as [OpenRouter](https://openrouter.ai/), [LiteLLM](https://litellm.ai/), [vLLM](https://docs.vllm.ai/), or [DeepSeek](https://api-docs.deepseek.com/), use a provider-specific package instead. See [Chat Completions API compatibility](/oss/python/integrations/chat#chat-completions-api) for details.
</Warning>
...
| :--- | :--- | :---: |  :---: | :---: | :---: |
| [`ChatOpenAI`](https://reference.langchain.com/python/langchain-openai/chat_models/base/ChatOpenAI) | [`langchain-openai`](https://reference.langchain.com/python/langchain-openai/) | beta | ✅ [(npm)](https://js.langchain.com/docs/integrations/chat/openai) | <a href="https://pypi.org/project/langchain-openai/" target="_blank"><img src="https://static.pepy.tech/badge/langchain-openai/month" alt="下载量 per month" noZoom height="100" class="rounded" /></a> | <a href="https://pypi.org/project/langchain-openai/" target="_blank"><img src="https://img.shields.io/pypi/v/langchain-openai?style=flat-square&label=%20&color=orange" alt="PyPI - Latest version" noZoom height="100" class="rounded" /></a> |

### 模型功能

| [Tool calling](/oss/python/langchain/tools) | [Structured output](/oss/python/langchain/structured-output) | 图像输入 | 音频输入 | 视频输入 | [Token-level streaming](/oss/python/langchain/streaming/) | 原生异步 | [Token usage](/oss/python/langchain/models#token-usage) | [Logprobs](/oss/python/langchain/models#log-probabilities) |
| :---: | :---: | :---: |  :---: | :---: | :---: | :---: | :---: | :---: |
| ✅ | ✅ | ✅ | ✅ | ❌ | ✅ | ✅ | ✅ | ✅ |

## 设置

要访问 OpenAI models you'll need to install the `langchain-openai` integration package and acquire an [OpenAI Platform](https://platform.openai.com) API key.
...
Head to the [OpenAI Platform](https://platform.openai.com/docs/api-reference/authentication) to sign up 并生成 API 密钥。 完成后设置 `OPENAI_API_KEY` environment variable in your environment:
...
3. `OPENAI_BASE_URL` — 由底层 `openai` SDK 客户端读取。LangChain 也会检查此变量以决定是否默认启用 `stream_usage`；当设置此变量时，默认不启用，因为许多非 OpenAI 端点不支持流式 Token 用量。

如果您想对模型调用进行自动追踪，还可以设置您的 [LangSmith](/langsmith/home) API key:
...
请参阅 [`ChatOpenAI`](https://reference.langchain.com/python/langchain-openai/chat_models/base/ChatOpenAI) API Reference for the full set of available model parameters.
...
OpenAI's Chat Completions API does not stream token usage statistics by default (请参阅 [OpenAI API reference for stream options](https://platform.openai.com/docs/api-reference/completions/create#completions-create-stream_options)).
...
<Note>
当使用 an async callable for the API key, you must use async methods (`ainvoke`, `astream`, etc.). Sync methods will raise an error.
</Note>
...
For more on binding tools and tool call outputs, 请前往 [tool calling](/oss/python/langchain/tools) docs.
...
请参阅 [streaming guide](/oss/python/langchain/streaming/) for more detail.
...
You can manage the state manually or using [LangGraph](/oss/python/langgraph/quickstart), as with other 聊天模型:
...
OpenAI has models that support multimodal inputs. You can pass in images, PDFs, or audio to these models. For more information on how to do this in LangChain, 请前往 [multimodal inputs](/oss/python/langchain/messages#multimodal) docs.
...
Note: OpenAI requires file-names be specified for PDF inputs. 当使用 LangChain's format, include the `filename` key.
...
更多信息，请参阅 OpenAI's [audio docs](https://platform.openai.com/docs/guides/audio).
</Note>
