...
在底层， [`create_agent`](https://reference.langchain.com/python/langchain/agents/factory/create_agent) is built on the basic agent loop -- calling a model, letting it choose tools to execute, and then finishing when it calls no more tools:
...
更多信息请参阅 [Agents](/oss/python/langchain/agents).
...
更多信息请参阅 [the complete middleware guide](/oss/python/langchain/middleware).
...
You don't need to learn LangGraph to use these features—they work 开箱即用.
...
更多信息请参阅 our guide on [content blocks](/oss/python/langchain/messages#standard-content-blocks).
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/python/releases/langchain-v1.mdx) 或[提交 issue](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>
