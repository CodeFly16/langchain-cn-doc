...
本指南演示了 LangGraph 图 API 的基础知识。它介绍了[状态](#define-and-update-state)，以及组合常见的图结构，如[序列](#create-a-sequence-of-steps)、[分支](#create-branches)和[循环](#create-and-control-loops)。它还涵盖了 LangGraph 的控制功能，包括用于 map-reduce 工作流的 [Send API](#map-reduce-and-the-send-api) 和用于将状态更新与跨节点"跳转"组合的 [Command API](#combine-control-flow-and-state-updates-with-command)。
...
注册 [LangSmith](https://smith.langchain.com?utm_source=docs&utm_medium=cta&utm_campaign=langsmith-signup&utm_content=oss-langgraph-use-graph-api) 以快速发现问题并提高 LangGraph 项目的性能。LangSmith 允许你使用追踪数据来调试、测试和监控用 LangGraph 构建的 LLM 应用——在[文档](/langsmith/observability)中了解更多入门信息。
</Tip>
...
Sometimes you want to be able to configure your graph when calling it. 例如, you might want to be able to specify what LLM or system prompt to use at runtime, _without polluting the graph state with these parameters_.
...
默认情况下, the retry policy retries on any exception except for the following:
...
<Note>
请注意 when issuing updates to the state, each node can just specify the value of the key it wishes to update.

默认情况下, this will **overwrite** the value of the corresponding key. You can also use [reducers](/oss/javascript/langgraph/graph-api#reducers) to control how updates are processed—例如, you can append successive updates to a key instead. See [Process state updates with reducers](#process-state-updates-with-reducers) for more detail.
</Note>
...
In this example, we fan out from `Node A` to `B and C` and then fan in to `D`. With our state, [we specify the reducer add operation](/oss/javascript/langgraph/graph-api#reducers). This will combine or accumulate values for the specific key in the State, rather than simply overwriting the existing value. For lists, this means concatenating the new list with the existing list. 请参阅 above section on [state reducers](#process-state-updates-with-reducers) for more detail on updating state with reducers.
...
<Tip>
Your conditional edges can route to multiple destination nodes. 例如:
...
Let's define a graph with a simple loop. 请注意 we use a conditional edge to implement a termination condition.
