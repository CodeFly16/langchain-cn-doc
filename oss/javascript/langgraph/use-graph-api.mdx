...
本指南演示了 LangGraph 图 API 的基础知识。它涵盖了[状态](#define-and-update-state)以及组合常见的图结构，如[序列](#create-a-sequence-of-steps)、[分支](#create-branches)和[循环](#create-and-control-loops)。它还涵盖了 LangGraph 的控制功能，包括用于 map-reduce 工作流的 [Send API](#map-reduce-and-the-send-api) 和用于将状态更新与跨节点"跳转"组合的 [Command API](#combine-control-flow-and-state-updates-with-command)。
