...
在生产环境中, use a checkpointer backed by a database:
...
You can configure subgraph-specific checkpointing behavior. See [subgraph persistence](/oss/javascript/langgraph/use-subgraphs#subgraph-persistence) 了解详情 on persistence levels including interrupt support and stateful continuations.
...
在生产环境中, use a store backed by a database:
...
<Accordion title="Long-term memory with semantic search">
    ...
    <Tabs>
      ...
      <Tab title="MongoDB (auto embedding)">
        <Note>
        Auto embedding requires MongoDB Atlas. MongoDB generates embeddings server-side via Voyage AI. 请参阅 [Automated Embedding documentation](https://www.mongodb.com/docs/atlas/atlas-vector-search/automated-embedding/) for more information.
        </Note>
...
<Warning>
When deleting messages, **make sure** that the resulting message history is valid. Check the limitations of the LLM provider you're using. 例如:
...
Prompting and orchestration logic can be used to summarize the message history. 例如, in LangGraph you can include a `summary` key in the state alongside the `messages` key:
