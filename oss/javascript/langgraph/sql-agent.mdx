...
LangChain 提供了内置的[智能体](/oss/javascript/langchain/agents)实现，使用 [LangGraph](/oss/javascript/langgraph/overview) 原语实现。如果需要更深层次的自定义，可以直接在 LangGraph 中实现智能体。本指南演示了一个 SQL 智能体的示例实现。如需实践入门，请参阅[使用更高层级的 LangChain 抽象构建 SQL 智能体](/oss/javascript/langchain/sql-agent)。

<Warning>
构建 SQL 数据库的问答系统需要执行模型生成的 SQL 查询。这样做存在固有风险。请确保你的数据库连接权限始终尽可能地限制在智能体需要的最小范围内。这将减轻（但不能消除）构建模型驱动系统的风险。
</Warning>

[预构建智能体](/oss/javascript/langchain/sql-agent)让我们能够快速上手，但我们依赖系统提示来约束其行为——例如，我们指示智能体始终从"列出表"工具开始，并始终在执行查询之前运行查询检查工具。

我们可以通过在 LangGraph 中自定义智能体来实施更高程度的控制。在这里，我们实现了一个简单的 ReAct 智能体设置，为特定的工具调用设置了专门的节点。我们将使用与预构建智能体相同的[状态]。
...
### LangSmith
设置 [LangSmith](https://smith.langchain.com?utm_source=docs&utm_medium=cta&utm_campaign=langsmith-signup&utm_content=oss-langgraph-sql-agent) 来检查你的链或智能体内部发生了什么。然后设置以下环境变量：
...
你将为本教程创建一个 [SQLite 数据库](https://www.sqlitetutorial.net/sqlite-sample-database/)。SQLite 是一个轻量级的数据库，易于设置和使用。我们将加载 `chinook` 数据库，这是一个代表数字媒体商店的示例数据库。

为了方便起见，我们将数据库（`Chinook.db`）托管在公共 GCS 存储桶中。
...
将这些步骤放在专门的节点中可以让我们 (1) 在需要时强制工具调用，以及 (2) 自定义与每个步骤关联的提示。
...
现在我们可以使用[图 API](/oss/javascript/langgraph/graph-api) 将这些步骤组装成工作流。我们在查询生成步骤定义了一个[条件边](/oss/javascript/langgraph/graph-api#conditional-edges)，如果生成了查询则路由到查询检查器，如果没有工具调用则结束（即 LLM 已经向查询交付了响应）。
...
在执行智能体的 SQL 查询之前检查它们以发现任何意外操作或效率问题是明智的做法。

这里我们利用 LangGraph 的[人机协作](/oss/javascript/langgraph/interrupts)特性在执行 SQL 查询之前暂停运行并等待人工审查。使用 LangGraph 的[持久化层](/oss/javascript/langgraph/persistence)，我们可以无限期地暂停运行（或至少在持久化层存活期间）。
...
<Note>
上述实现遵循了更广泛的[人机协作](/oss/javascript/langgraph/interrupts)指南中的[工具中断示例](/oss/javascript/langgraph/interrupts#interrupts-in-tools)。有关详细信息和替代方案，请参阅该指南。
</Note>

现在让我们重新组装图。我们将用人工审查替换程序化检查。请注意，我们现在包含了一个[检查点器](/oss/javascript/langgraph/persistence)；这是暂停和恢复运行所必需的。
