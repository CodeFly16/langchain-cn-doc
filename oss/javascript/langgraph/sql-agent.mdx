...
在本教程中，我们将使用 LangGraph 构建一个可以回答 SQL 数据库相关问题的自定义智能体。

LangChain 提供了内置的[智能体](/oss/javascript/langchain/agents)实现，使用 [LangGraph](/oss/javascript/langgraph/overview) 原语实现。如果需要更深度的自定义，可以直接在 LangGraph 中实现智能体。本指南演示了一个 SQL 智能体的示例实现。有关实用介绍，请参阅[使用更高层级的 LangChain 抽象构建 SQL 智能体](/oss/javascript/langchain/sql-agent)。

<Warning>
构建 SQL 数据库的问答系统需要执行模型生成的 SQL 查询。这样做存在固有风险。请确保您的数据库连接权限始终尽可能窄地限定在智能体需要的范围内。这将减轻但不能完全消除构建模型驱动系统的风险。
</Warning>

[预构建智能体](/oss/javascript/langchain/sql-agent)让我们能够快速上手，但我们依赖系统提示来约束其行为——例如，我们指示智能体始终从"列出表"工具开始，并始终在执行查询之前运行查询检查工具。

我们可以在 LangGraph 中通过自定义智能体来实施更高程度的控制。在这里，我们实现一个简单的 ReAct 智能体设置，为特定的工具调用设置专用节点。我们将使用与预构建智能体相同的[状态]。

### 概念

我们将涵盖以下概念：

- 用于读取 SQL 数据库的[工具](/oss/javascript/langchain/tools)
...
### LangSmith
设置 [LangSmith](https://smith.langchain.com?utm_source=docs&utm_medium=cta&utm_campaign=langsmith-signup&utm_content=oss-langgraph-sql-agent) 以检查您的链或智能体内部发生了什么。然后设置以下环境变量：
...
我们将使用 `@langchain/classic/sql_db` 模块中的便捷 SQL 数据库包装器来与数据库交互。该包装器提供了执行 SQL 查询和获取结果的简单接口：
...
将这些步骤放在专用节点中让我们能够 (1) 在需要时强制执行工具调用，以及 (2) 自定义与每个步骤关联的提示。
...
我们现在可以使用[图 API](/oss/javascript/langgraph/graph-api) 将这些步骤组装成工作流。我们在查询生成步骤定义一个[条件边](/oss/javascript/langgraph/graph-api#conditional-edges)，如果生成了查询则路由到查询检查器，如果没有工具调用则结束（即大语言模型(LLM)已经对查询给出了回答）。
...
<Tip>
查看上述运行的 [LangSmith 追踪](https://smith.langchain.com/public/a6a96896-686a-4040-b9b5-28d701453d6f/r)。
</Tip>


## 6. 实现人机协作审查

在执行 SQL 查询之前检查智能体的查询是否存在意外操作或低效是很谨慎的做法。

这里我们利用 LangGraph 的[人机协作](/oss/javascript/langgraph/interrupts)功能，在执行 SQL 查询之前暂停运行并等待人工审查。使用 LangGraph 的[持久化层](/oss/javascript/langgraph/persistence)，我们可以无限期暂停运行（或者至少在持久化层存活的时间内）。

让我们用一个接收人工输入的节点来包装 `sql_db_query` 工具。我们可以使用 [interrupt](/oss/javascript/langgraph/interrupts) 函数来实现这一点。下面，我们允许输入来批准工具调用、编辑其参数或提供用户反馈。
...
<Note>
上述实现遵循更广泛的[人机协作](/oss/javascript/langgraph/interrupts)指南中的[工具中断示例](/oss/javascript/langgraph/interrupts#interrupts-in-tools)。请参阅该指南了解详情和替代方案。
</Note>

让我们现在重新组装图。我们将用人工审查替换程序化检查。注意，我们现在包含了一个[检查点器](/oss/javascript/langgraph/persistence)；这是暂停和恢复运行所必需的。
...
请参阅[人机协作指南](/oss/javascript/langgraph/interrupts)了解详情。
