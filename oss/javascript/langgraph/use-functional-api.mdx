...
[**函数式 API**](/oss/javascript/langgraph/functional-api) 允许你以最少的代码更改将 LangGraph 的关键功能（[持久化](/oss/javascript/langgraph/persistence)、[记忆](/oss/javascript/langgraph/add-memory)、[人机协作](/oss/javascript/langgraph/interrupts)和[流式输出](/oss/javascript/langgraph/streaming)）添加到现有应用中。
