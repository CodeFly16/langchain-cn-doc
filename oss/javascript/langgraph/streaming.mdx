...
<Info>
**预览中：** 尝试事件流式输出对消息、状态、子图、输出和自定义扩展的类型化投影。从[事件流式输出摘要](/oss/javascript/langgraph/streaming/event-streaming)开始，或探索[流式输出 cookbook](https://github.com/langchain-ai/streaming-cookbook) 中的可运行示例。
</Info>

LangGraph 实现了一个流式输出系统来呈现实时更新。流式输出对于增强基于 LLM 构建的应用程序的响应性至关重要。通过逐步显示输出，即使在完整响应准备好之前，流式输出也能显著改善用户体验（UX），特别是在处理 LLM 延迟时。
...
LangGraph 图通过 [`stream`](https://reference.langchain.com/javascript/classes/_langchain_langgraph.pregel.Pregel.html#stream) 方法将流式输出作为迭代器返回。
...
| [updates](#graph-state) | 每一步后的状态更新。同一步骤中的多个更新分别流式传输。 |
...
* `updates` 在图的每一步后流式传输状态的**更新**。
* `values` 在图的每一步后流式传输状态的**完整值**。
...
<Tabs>
    <Tab title="updates">
    使用此模式仅流式传输每步后节点返回的**状态更新**。流式输出包括节点名称以及更新内容。
...
使用 `messages` 流式模式从图的任何部分（包括节点、工具、子图或任务）**逐 Token** 流式传输大语言模型（LLM）输出。
...
> 如果你的 LLM 没有 LangChain 集成，你可以使用 `custom` 模式流式传输其输出。详见[与任意 LLM 配合使用](#use-with-any-llm)。
...
const callModel: GraphNode<typeof MyState> = async (state) => {
  ...
  // 请注意 message events are emitted even when the LLM is run using .invoke rather than .stream
...
使用 `nostream` 标签将 LLM 输出完全排除在流之外。带有 `nostream` 标签的调用仍然运行并产生输出；它们的 Token 只是不会在 `messages` 模式中发出。

This is useful when:

- You need LLM output for internal processing (例如 structured output) but do not want to stream it to the client
- You stream the same content through a different channel (例如 custom UI messages) and want to avoid duplicate output in the `messages` stream
...
使用 `tools` 流式模式接收工具执行的实时生命周期事件。这对于在工具运行时在 UI 中显示进度指示器、部分结果和错误状态非常有用。
...
来自 `@langchain/langgraph-sdk/react` 的 `useStream` 钩子在你的流式模式中包含 `"tools"` 时会暴露一个 `toolProgress` 数组。每个条目是一个 `ToolProgress` 对象，用于追踪正在运行的工具的当前状态：
...
<Accordion title="Extended example: streaming from subgraphs">
  ...
  const SubgraphState = new StateSchema({
    foo: z.string(), // 请注意 this key is shared with the parent graph state
    bar: z.string(),
  });
...
使用 `debug` 流式模式在图执行过程中尽可能多地流式传输信息。流式输出包括节点名称以及完整状态。
...
你可以将数组作为 `streamMode` 参数传递，以同时流式传输多种模式。
...
这使你能够集成原始 LLM 客户端或提供自己流式接口的外部服务，使 LangGraph 对于自定义设置具有高度灵活性。
...
<Note>
并非所有聊天模型集成都支持 `streaming` 参数。如果你的模型不支持它，请改用 `disableStreaming: true`。此参数通过基类在所有聊天模型上可用。
</Note>
