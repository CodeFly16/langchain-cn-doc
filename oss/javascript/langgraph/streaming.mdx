...
LangGraph 实现了一个流式输出系统来呈现实时更新。流式输出对于增强基于大语言模型(LLM)构建的应用程序的响应性至关重要。通过逐步显示输出，甚至在完整响应准备好之前，流式输出显著改善了用户体验（UX），特别是在处理大语言模型(LLM)的延迟时。
...
| [updates](#graph-state) | 每步后的状态更新。同一步骤中的多个更新分别流式输出。 |
| [messages](#llm-tokens) | 来自大语言模型(LLM)调用的 (LLM Token, 元数据) 二元组。 |
