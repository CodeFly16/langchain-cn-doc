...
<Accordion title="Full example: different state schemas">
  ...
  // Define subgraph
  const SubgraphState = new StateSchema({
    // 请注意 none of these keys are shared with the parent graph state
    bar: z.string(),
    baz: z.string(),
  });
...
When the parent graph and subgraph **share state keys**, you can pass a compiled subgraph directly to `add_node`. No wrapper function is needed—the subgraph reads from and writes to the parent's state channels automatically. 例如, in [multi-agent](/oss/javascript/langchain/multi-agent) systems, the agents often communicate over a shared [messages](/oss/javascript/langgraph/graph-api#why-use-messages) key.
...
<Accordion title="Full example: shared state schemas">
  ...
  const subgraphBuilder = new StateGraph(SubgraphState)
    ...
    .addNode("subgraphNode2", (state) => {
      // 请注意 this node is using a state key ('bar') that is only available in the subgraph
...
以下示例s use two subagents (fruit expert, veggie expert) wrapped as tools for an outer agent:
...
以下示例s use a fruit expert subagent compiled with `checkpointer=True`:
