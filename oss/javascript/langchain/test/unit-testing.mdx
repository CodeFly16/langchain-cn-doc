...
[`fakeModel`](https://reference.langchain.com/javascript/langchain/index/fakeModel) 是一个构建器风格的模拟聊天模型，允许你编写精确的响应（文本、工具调用、错误）并断言模型接收到的内容。它扩展了 [`BaseChatModel`](https://reference.langchain.com/javascript/langchain-core/language_models/chat_models/BaseChatModel)，因此可以在任何期望使用真实模型的地方使用。
...
<Tip>
    `.respond()` 和 `.respondWithTools()` 可以自由混合使用。这在测试智能体循环时特别有用，因为模型会在工具调用和文本响应之间交替。
</Tip>
...
向 `.respond()` 传递一个 `Error` 会使模型在该特定调用时抛出异常。错误可以出现在序列中的任何位置：
...
const model = fakeModel()
  .respond(new Error("rate limit exceeded"))  // 第1轮: 抛出异常  // [!code highlight]
...
`.respond()` 也接受一个基于输入消息计算响应的函数。该函数接收完整的消息数组，并返回 [`BaseMessage`](https://reference.langchain.com/javascript/langchain-core/messages/BaseMessage) 或 `Error`：
...
了解如何使用真实模型提供商 API 测试你的智能体，请参阅[集成测试](/oss/javascript/langchain/test/integration-testing)。
