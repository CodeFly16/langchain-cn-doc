...
The `load_skill` tool returns the full skill content as a string, which becomes part of the conversation as a ToolMessage. 有关更多详情，请参阅 creating and using tools, see the [Tools guide](/oss/javascript/langchain/tools).
...
<Note>
**Production consideration**: This tutorial loads the skill list in `__init__` for simplicity. In a production system, you may want to load skills in the `before_agent` hook instead, allowing them to be refreshed periodically to reflect up-to-date changes (e.g., when new skills are added or existing ones are modified). 请参阅 [before_agent hook documentation](/oss/javascript/langchain/middleware/custom#node-style-hooks) 了解详情.
</Note>
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/langchain/multi-agent/skills-sql-assistant.mdx) 或[提交 Issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
</div>
