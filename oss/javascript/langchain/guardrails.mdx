...
你可以使用[中间件](/oss/javascript/langchain/middleware)在关键位置拦截执行来实现护栏——在智能体启动之前、完成之后，或围绕模型和工具调用。
...
LangChain 提供了内置护栏（如 [PII 检测](#pii-detection)、[人机协作](#human-in-the-loop)）以及灵活的中间件系统，用于使用任一方法构建自定义护栏。
...
有关 PII 检测功能的完整详情，请参阅[中间件文档](/oss/javascript/langchain/middleware#pii-detection)。
...
<Tip>
    有关实现审批工作流的完整详情，请参阅[人机协作文档](/oss/javascript/langchain/human-in-the-loop)。
</Tip>
...
- [人机协作](/oss/javascript/langchain/human-in-the-loop) - 为敏感操作添加人工审查
- [测试智能体](/oss/javascript/langchain/test/) - 安全机制测试策略
