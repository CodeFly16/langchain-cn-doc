---
title: 加入与重新加入流
description: 从运行中的智能体流断开连接并重新连接
---

加入和重新加入功能让你可以从运行中的智能体流断开连接而不停止智能体，然后稍后重新连接。智能体在客户端离开期间继续在服务器端执行，你可以从断开的位置准确地恢复流。
...
传统的流式 API 将客户端和服务器紧密耦合：如果客户端断开连接，流就会丢失。加入与重新加入打破了这种耦合，实现了几种重要的模式：

- **网络中断**：在蜂窝基站或 Wi-Fi 网络之间切换的移动用户可以无缝恢复
- **页面导航**：用户从聊天页面导航离开后返回，不会丢失进度
...
| `streamResumable: true` | 提交选项，使流可以在之后被重新加入 |

<Note>
`stream.stop()` 与取消运行有本质区别。停止只是断开**客户端**连接。智能体继续在服务器端处理。要真正取消智能体的执行，你应该使用中断或取消机制。
</Note>
...
提交消息时，传递 `onDisconnect: "continue"` 和 `streamResumable: true` 来启用加入/重新加入流程：
...
| 选项 | 默认值 | 描述 |
|---|---|---|
| `onDisconnect` | `"cancel"` | 客户端断开时的行为。`"continue"` 保持智能体运行；`"cancel"` 停止它。 |
| `streamResumable` | `false` | 设为 `true` 时，服务器保留流状态，以便客户端稍后可以重新加入。 |

<Tip>
始终同时使用这两个选项。设置 `onDisconnect: "continue"` 而不设置 `streamResumable: true` 意味着智能体继续运行，但你无法重新加入流来查看其输出。
</Tip>
...
使用保存的运行 ID 调用 `stream.joinStream(runId)` 来重新连接：
...
对于跨会话重新加入（例如用户关闭浏览器后返回），将运行 ID 持久化到存储中：
...
<Info>
持久化的运行 ID 应在运行完成时清理。监听流完成事件并移除存储的 ID，以避免尝试重新加入已完成的运行。
</Info>

## 错误处理

如果运行已过期、已删除或服务器已重启，重新加入可能会失败。优雅地处理这些情况：
...
- **始终保存运行 ID**：没有它就无法重新加入。同时使用组件状态和持久化存储以增强可靠性。
- **清晰显示连接状态**：用户应始终知道他们是在接收实时更新还是在查看快照。
- **在可见性变化时自动重新加入**：使用 Page Visibility API 在用户返回标签页时自动重新加入。
- **设置合理的超时**：如果重新加入尝试时间过长，回退到获取线程历史记录。
- **清理已完成的运行**：在智能体完成时移除持久化的运行 ID，以避免过期的重新加入尝试。
