...
推理 Token 暴露了高级模型（如 OpenAI 的 o1/o3 和 Anthropic 的 Claude 扩展思考）的内部思考过程。这些模型生成结构化内容块，将推理与最终答案分离，让你构建展示模型*如何*得出响应的 UI。
...
当具有推理能力的模型处理提示词时，它们会生成两种不同类型的内容：

1. **推理块**：模型的内部思维链、问题分解和逐步分析
2. **文本块**：呈现给用户的最终精炼响应

这些内容以类型化内容块的形式在 `AIMessage` 中传递，可通过 `contentBlocks` 属性访问：
...
<Note>
并非所有模型都会生成推理 Token。此模式专门适用于支持扩展思考或思维链输出的模型。标准聊天模型仅返回文本块。
</Note>
...
单条消息可能包含多个推理块（例如，如果模型暂停推理、生成部分文本，然后继续推理）。将它们连接起来可以获得完整的思考过程。
...
`ThinkingBubble` 在视觉上有区分的可折叠容器中呈现推理 Token。用户可以展开查看完整的思考过程，或折叠以专注于最终答案。
...
### 设置 ThinkingBubble 样式

用独特的视觉处理方式将推理块与常规消息区分开来：
...
当模型仍在生成推理 Token 时，显示一个动画指示器来表明思考正在进行中：
...
<Tip>
在流式输出期间，默认保持 ThinkingBubble 折叠，仅显示加载动画。在流式输出中途展开可能会因为新 Token 到达而导致布局抖动。让用户在推理阶段完成后再展开。
</Tip>
...
并非每条 AI 消息都包含推理块。当 `contentBlocks` 只有文本块时，渲染标准消息气泡而不使用 ThinkingBubble。

### 空的推理块

某些模型会生成空的推理块作为占位符。过滤掉它们：
...
单条消息可以在推理块和文本块之间交替。如果你需要保留这种交错，按顺序遍历 `contentBlocks` 而不是按类型分组：
...
- **视觉上区分**：使用不同的颜色、边框或背景，使推理永远不会与实际答案混淆
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/langchain/frontend/reasoning-tokens.mdx) 或 [提交 issue](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>
