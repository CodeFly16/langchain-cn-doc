...
[OpenUI](https://github.com/openuidev) 是一个生成式 UI 库，让语言模型以称为 **openui-lang** 的声明式格式生成完整的交互式 UI。智能体不是返回聊天消息，而是返回包含卡片、图表、表格、标签和表单的组件树，`Renderer` 将其转换为真实的 React UI。

这种集成非常适合数据丰富的输出，如报告、仪表板和数据浏览器，其中模型既是数据分析师又是 UI 设计师。
...
将 `useStream` 直接连接到 `Renderer` 会导致每个流式 Token 都重新渲染，产生数百次无效的重新解析。这会导致图表组件在数据尚未到达时崩溃。以下工具解决了这些问题：

| 问题 | 解决方案 |
| --- | --- |
| **部分字符串字面量** | `truncateAtOpenString` / `closeOrTruncateOpenString` — 在解析前删除或关闭不完整的字符串 |
| **Token 中间的抖动** | `useStableText` — 在完整语句边界（`name = Expr(…)`）而不是每个 Token 上更新 Renderer |
| **图表空数据崩溃** | `chartDataRefsResolved` — 在包含之前验证图表的 `Series` 和标签数组已定义 |
| **尚无 `root` / 回退** | `buildProgressiveRoot` — 当模型尚未写入 `root = Stack(…)` 时从顶级变量合成一个 |
| **snake_case 标识符** | `sanitizeIdentifiers` — 解析器只接受 camelCase；转换模型输出的任何 `snake_case` 名称 |
 ...
 * 转换模型输出的任何 snake_case 变量名；字符串内容不受影响。
 */
...
/**
 * 遍历文本跟踪打开的字符串。如果文本在字符串中间结束，截断到
 * 最后一个安全换行符——这防止部分字符串字面量吞噬
 * 我们之后合成的任何 `root = Stack(…)` 行。
 */
...
/**
 * 图表组件（recharts）在标签或 series 属性未解析时会因 `.map() on null` 崩溃。
 * 在提交稳定快照之前，验证文本中每个图表的所有数据变量已定义。
 */
...
/**
 * 如果模型尚未写入 `root = Stack(…)`，则从顶级变量
 * （已定义但未在其他表达式中引用的变量）合成一个。
 * 即使模型最后才写 root，也能启用渐进式渲染。
 */
...
OpenUI 的 `Button` 组件支持 `continue_conversation` 操作类型。当用户点击后续按钮时，`Renderer` 触发 `onAction`，上面的 `AIMessageView` 将按钮标签作为下一条用户消息提交，与在输入框中输入的代码路径完全相同。

通过系统提示词中的 `additionalRules` 在每个报告末尾添加"深入探索"部分：
