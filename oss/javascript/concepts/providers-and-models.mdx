...
Every LangChain chat model, regardless of provider, implements the same interface. 这意味着 you can:
...
In LangChain, each provider has a dedicated **integration package** (for example `langchain-openai`, `langchain-anthropic`) that implements the standard LangChain interface for that provider's models. 这意味着:
...
<CardGroup cols={2}>
    ...
    <Card title="All providers" icon="grid-dots" href="/oss/javascript/integrations/providers/overview">
        请参阅full list of provider packages and integrations.
    </Card>
