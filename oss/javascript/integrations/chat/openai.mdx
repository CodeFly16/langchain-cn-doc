...
title: "ChatOpenAI 集成"
sidebarTitle: "聊天"
description: "使用 LangChain JavaScript 集成 ChatOpenAI chat model。"
---

[OpenAI](https://en.wikipedia.org/wiki/OpenAI) 是一个人工智能 (AI) 研究实验室。

本指南将帮助你开始使用 OpenAI [chat models](/oss/javascript/langchain/models)。有关所有 `ChatOpenAI` 功能和配置的详细文档，请前往 [API 参考](https://reference.langchain.com/javascript/langchain-openai/ChatOpenAI)。
...
前往 [OpenAI's website](https://platform.openai.com/) 注册 OpenAI 并生成 API 密钥。完成后设置 `OPENAI_API_KEY` 环境变量：
...
LangChain 的 [`ChatOpenAI`](https://reference.langchain.com/javascript/langchain-openai/ChatOpenAI) 集成位于 `@langchain/openai` 包中：
...
console.log({
  ...castAudioContent,
  data: castAudioContent.data.slice(0, 100) // 为简洁起见截断
})
```
...
console.log({
  ...
  data: castStreamedAudioContent.data.slice(0, 100) // 为简洁起见截断
})
```
...
const userInput = new HumanMessage({
  content: [{
    type: "input_audio",
    input_audio: {
      data: castAudioContent.data, // 复用第一个示例中的 base64 数据
      format: "wav",
    },
  }]
})
...
<Callout icon="edit">
        [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/javascript/integrations/chat/openai.mdx) 或 [提交 issue](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>
