...
This migration guide outlines the major changes in LangChain v1. 要了解更多关于 the new features of v1, see the [introductory post](/oss/javascript/releases/langchain-v1).
...
请参阅 content blocks [reference](/oss/javascript/langchain/messages#content-block-reference) 了解更多详情.

### Serialize standard content

Standard content blocks are **not serialized** into the `content` attribute 默认情况下. If you need to access standard content blocks in the `content` attribute (e.g., when sending messages to a client), you can opt-in to serializing them into `content`.
...
Legacy functionality outside the focus of standard interfaces and agents has been moved to the [`@langchain/classic`](https://www.npmjs.com/package/@langchain/classic) package. 请参阅 [Simplified package](#simplified-package) section 了解详情 on what's available in the core `langchain` package and what moved to `@langchain/classic`.
