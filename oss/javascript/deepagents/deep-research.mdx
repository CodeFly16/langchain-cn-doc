...
title: 构建深度研究智能体
sidebarTitle: 深度研究
description: 构建一个具有子智能体委派功能的多步骤网络研究智能体
---
...
本指南 demonstrates how to build a multi-step web research agent from scratch using [Deep Agents](/oss/javascript/deepagents). 智能体将研究问题分解为集中的任务, 将它们委派给专门的子智能体, 并将发现综合成一份全面的报告。

你构建的智能体将：

1. 使用待办列表规划研究
1. 将集中的研究任务委派给具有隔离上下文的子智能体
1. 评估搜索结果并规划后续步骤 as you gather information
1. 将带有适当引用的发现综合成最终报告
...
- Custom [tools](/oss/javascript/langchain/tools) 用于网络搜索
...
- [Tavily](https://www.tavily.com/) 用于网络搜索 (可选 - 免费层级 sufficient)
...
你可以使用 the tavily_search tool to find resources that can help answer the research question.
...
You can run the agent synchronously, meaning it will wait了解完整的result and then print it, or you can stream updates as they come in.
...
View the complete [Deep Research example](https://github.com/langchain-ai/deepagents/tree/main/examples/deep_research) 在 GitHub 上.
...
你也可以 tune the delegation limits to allow了解更多parallel sub-agents or delegation rounds.
