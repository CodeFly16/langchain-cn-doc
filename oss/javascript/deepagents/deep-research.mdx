...
本指南演示如何使用[深度智能体](/oss/javascript/deepagents)从零构建一个多步骤网络研究智能体。该智能体将研究问题分解为聚焦的任务，委派给专门的子智能体，并将研究发现合成为综合报告。

你构建的智能体将：

1. 使用待办事项列表规划研究
1. 将聚焦的研究任务委托给具有隔离上下文的子智能体
1. 在收集信息时评估搜索结果并规划下一步
1. 将研究发现连同正确的引用合成为最终报告
...
- [Tavily](https://www.tavily.com/) 用于网络搜索（可选 - 免费层级即可满足）
...
你可以同步运行智能体，即等待完整结果后打印，或者在更新到来时流式输出。



将相应标签页的代码添加到 `agent.ts` 底部：
...
如果你在运行前设置了 `LANGSMITH_API_KEY` 环境变量，可以在 [LangSmith](/langsmith/home) 中查看智能体的追踪记录，以调试和监控多步骤行为。
...
现在你已经构建了智能体，可以通过更改智能体文件中的提示常量来自定义它，以调整工作流、委派策略或研究者行为。
你还可以调整委派限制以允许更多的并行子智能体或委派轮次。

有关本教程中概念的更多信息，请查看以下资源：

- [子智能体](/oss/javascript/deepagents/subagents)：了解如何使用不同工具和提示配置子智能体
