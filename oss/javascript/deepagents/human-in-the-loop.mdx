...
某些工具操作可能是敏感的，需要在执行前获得人工审批。 深度智能体通过 LangGraph 的中断功能支持人机协作工作流。 你可以使用 `interrupt_on` 参数配置哪些工具需要审批。
...
const requestApproval = tool(
  async ({ action描述 }: { action描述: string }) => {
    const approval = interrupt({
      type: "approval_request",
      action: action描述,
      message: `Please approve or reject: ${action描述}`,
    }) as { approved?: boolean; reason?: string };

    if (approval.approved) {
      return `Action '${action描述}' was APPROVED. Proceeding...`;
    } else {
      return `Action '${action描述}' was REJECTED. Reason: ${
