---
title: 走向生产环境
description: 通过持久化记忆、沙箱、弹性中间件和部署选项将深度智能体投入生产
---
...
本指南 covers considerations for taking a deep agent from a local prototype to a production deployment. 它将引导你完成记忆范围界定、配置执行环境、添加防护栏和连接前端。


## 概述

Agents use information from memory and their execution environment to accomplish tasks.
在生产环境中, there are a few primitives that determine how information is shared and accessed:
...
有关完整的 set of configuration options (custom Docker steps, store indexing, auth handlers, and more), see [application structure](/oss/javascript/langgraph/application-structure).
...
How you [scope memory](#scoping) and [execution environments](#execution-environment) determines what data is shared between users. 请参阅sections below了解详情.
...
Custom roles with granular permissions are available on Enterprise plans. 请参阅[RBAC reference](/langsmith/rbac)了解完整的permission model.
...
**Workspace secrets.** For API keys shared across all users (for example your organization's LLM provider keys, search API keys), store them as [workspace secrets](/langsmith/set-up-hierarchy#configure-workspace-settings) in LangSmith. See [Managing secrets](#managing-secrets)了解详情.
...
你也可以 read and write to the store from your application code using the [Store API](/langsmith/custom-store). See [Advanced usage](/oss/javascript/deepagents/memory#advanced-usage) for examples.

有关完整的 namespace factory API, see [namespace factories](/oss/javascript/deepagents/backends#namespace-factories). For memory patterns like self-improving instructions and knowledge bases, see [long-term memory](/oss/javascript/deepagents/memory).

## 执行环境

Locally, agents can read and write files on disk and run shell commands directly. 在生产环境中, you need to think about isolation and persistence. The right setup depends on whether your agent needs to execute code:
...
- [CompositeBackend](https://reference.langchain.com/javascript/deepagents/backends/CompositeBackend): mix both. 线程作用域 scratch space by default with cross-thread routes for specific paths like `/memories/`.

有关完整的 list of backends and how to build custom ones, see [backends](/oss/javascript/deepagents/backends).
...
|---|---|---|---|
| **线程作用域** | [Thread](/langsmith/use-threads) metadata | Fresh per conversation, cleaned up on TTL | A data analysis bot where each conversation starts clean |
...
```
  ...
  <Tab title="Assistant-scoped">
    All threads share one sandbox. 这在以下情况很有用 when the sandbox has state that's expensive to recreate, such as a cloned repo, installed dependencies, or build artifacts. Any conversation on the same assistant picks up where the last one left off without repeating setup.
...
for await (const chunk of client.runs.stream(
  thread1.thread_id,
  "agent",
  { input: { messages: [{ role: "human", content: "Clone https://github.com/org/repo and 安装依赖" }] } },
)) {
  console.log(chunk.data);
}
...
Strategies include `redact` (replace with `[REDACTED_EMAIL]`), `mask` (partial masking like `****-****-****-1234`), `hash` (deterministic hash), and `block` (raise an error). 你也可以 write custom detectors for domain-specific patterns.

See [piiMiddleware](https://reference.langchain.com/javascript/langchain/index/piiMiddleware)了解完整的configuration.
...
Locally, `useStream` points at `http://localhost:2024`. 在生产环境中, point it at your [LangSmith Deployment](/langsmith/deployment) and configure reconnection so users don't lose progress if their connection drops.
