...
本指南涵盖将深度智能体从本地原型部署到生产环境的注意事项。它逐步介绍了记忆范围界定、执行环境配置、添加防护措施和连接前端。


## 概述

智能体使用来自记忆和执行环境的信息来完成任务。
...
| 范围 | 命名空间 | 使用场景 | 示例 |
| --- | --- | --- | --- |
| **用户**（推荐默认值） | `(user_id)` | Per-user preferences and context | "I prefer concise responses" |
| **助手** | `(assistant_id)` | Shared instructions for one assistant | "Cap posts at 280 characters" |
| **全局** | `(org_id)` | Read-only policies for all users and assistants | "Never disclose internal pricing" |
...
| 范围 | 沙箱 ID 存储位置 | 生命周期 | 示例使用场景 |
|---|---|---|---|
| **线程范围** | [Thread](/langsmith/use-threads) metadata | Fresh per conversation, cleaned up on TTL | A data analysis bot where each conversation starts clean |
| **助手范围** | [Assistant](/langsmith/assistants) config | Shared across all conversations | A coding assistant that maintains a cloned repo across conversations |
...
<Callout icon="edit">
    [在 GitHub 上编辑此页面](https://github.com/langchain-ai/docs/edit/main/src/oss/deepagents/going-to-production.mdx)或[提交 issue](https://github.com/langchain-ai/docs/issues/new/choose)。
</Callout>
</div>
