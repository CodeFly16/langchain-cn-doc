...
你可以使用 [Agent Skills](https://agentskills.io/) to provide your deep agent with new capabilities and expertise. For ready-to-use skills that improve your agent's performance on LangChain ecosystem tasks, see the [LangChain Skills](https://github.com/langchain-ai/langchain-skills) repository.
...
- Additional scripts (可选)
- Additional reference info, such as docs (可选)
- Additional assets, such as templates and other resources (可选)
...
以下 example shows a skill that gives instructions on how to provide relevant langgraph docs when prompted:
...
以下 example shows a `SKILL.md` file using all available frontmatter fields:
...
<ParamField body="skills" type="list[str]" 可选>
...
- [Execute skill scripts in a sandbox](#execute-skill-scripts-in-a-sandbox) when the agent needs to 安装依赖, run tests, call CLIs, or work with an operating-system filesystem.
