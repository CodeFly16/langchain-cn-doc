...
本页面 provides an overview of the different kinds of context your deep agent has access to and manages.

<Tip>
New to context engineering? 请参阅[conceptual overview](/oss/javascript/concepts/context) for the different types of context and when to use them.
</Tip>
...
输入上下文是在启动时提供给深度智能体的信息 它成为系统提示的一部分。 The final prompt consists of several sources:
...
你的自定义系统提示被添加到内置系统提示之前, which includes guidance for planning, filesystem tools, and subagents. Use it to define the agent's role, behavior, and knowledge:
...
**Tools you provide** – Tools passed via the `tools` parameter get their descriptions (from the tool schema) sent to the model. 你也可以 add [custom middleware](/oss/javascript/langchain/middleware) that adds tools and appends its own system prompt instructions.
...
1. 虚拟文件系统 prompt (filesystem + execute tool docs if applicable)
...
以下 techniques are the built-in mechanisms to ensure the context passed to LLMs stays within its context window limit:
...
例如, you may prompt the agent to store preferences in `/memories/preferences.txt`.
...
- [Harness](/oss/javascript/deepagents/harness) – 上下文管理 overview, offloading, summarization
