...
深度智能体可以创建子智能体来委派工作。 你可以在 `subagents` 参数配置哪些工具需要审批。 子智能体对于以下场景很有用 [上下文隔离](https://www.dbreunig.com/2025/06/26/how-to-fix-your-context.html#context-quarantine) (保持主智能体的上下文整洁) 以及提供专门的指令。

本页面 covers **synchronous** subagents, where the supervisor blocks until the subagent finishes. For long-running tasks, parallel workstreams, or cases where you need mid-flight steering and cancellation, see [Async subagents](/oss/javascript/deepagents/async-subagents).
...
| Field | Type | 描述 |
|-------|------|-------------|
| `name` | `str` | 必需. Unique identifier for the subagent. The main agent uses this name when calling the `task()` tool. The subagent name becomes metadata for `AIMessage`s and for streaming, which helps to differentiate between agents. |
| `description` | `str` | 必需. 描述 of what this subagent does. Be specific and action-oriented. The main agent uses this to decide when to delegate. |
| `system_prompt` | `str` | 必需. Instructions for the subagent. Custom subagents must define their own. Include tool usage guidance and output format requirements.<br></br>Does not inherit from main agent. |
...
<Tip>
    **CLI users:** 你也可以 define subagents as `AGENTS.md` files on disk instead of in code. The `name`, `description`, and `model` fields map to YAML frontmatter, and the markdown body becomes the `system_prompt`. See [Use subagents in the CLI](/oss/javascript/deepagents/cli/subagents) for the file format.

    **Deploy users:** Define subagents as directories under `subagents/` with their own `deepagents.toml` and `AGENTS.md`. The bundler auto-discovers them. See [Deploy subagents](/oss/javascript/deepagents/deploy#subagents)了解完整的configuration reference.
</Tip>
...
| Field | Type | 描述 |
|-------|------|-------------|
| `name` | `str` | 必需. Unique identifier for the subagent. The subagent name becomes metadata for `AIMessage`s and for streaming, which helps to differentiate between agents. |
| `description` | `str` | 必需. What this subagent does. |
| `runnable` | `Runnable` | 必需. A compiled LangGraph graph (must call `.compile()` first). |
...
以下 example creates a deep agent with the name `main-agent` and a subagent with the name `research-agent`:
...
这意味着 tools running inside any subagent can access the same context values you provided to the parent:
...
const contextSchema = z.object({
  userId: z.string(),
  researcherMaxDepth: z.number().可选(),
  factCheckerStrictMode: z.boolean().可选(),
});
