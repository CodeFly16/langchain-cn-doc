...
- **`~/.agents/`** — 工具无关数据（在 AI CLI 工具之间共享的技能）
...
Each subagent is an `AGENTS.md` file with YAML frontmatter (`name`, `description`, 可选 `model`) and a markdown body for the system prompt. See [Use subagents in the CLI](/oss/javascript/deepagents/cli/subagents)了解完整的format reference.
