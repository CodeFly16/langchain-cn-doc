...
除了用于规划、文件管理和子智能体启动的[内置工具](/oss/javascript/deepagents/overview#core-capabilities)外，你还可以提供自定义工具：
...
深度智能体支持任何[中间件](/oss/javascript/langchain/middleware/overview), including the built-in middleware listed below, prebuilt middleware from LangChain, provider-specific middleware, and custom middleware you write yourself. Pass middleware to the `middleware` argument of `create_deep_agent`.
...
深度智能体的工具可以利用虚拟文件系统 to store, access, and edit files. By default, deep agents use a [`StateBackend`](https://reference.langchain.com/javascript/deepagents/backends/StateBackend).
