...
有关完整的 parameter list, see the [`createDeepAgent`](https://reference.langchain.com/javascript/deepagents/types/CreateDeepAgentParams) API reference.
...
const internetSearch = tool(
  ...
  },
  {
    ...
    schema: z.object({
      ...
      maxResults: z.number().可选().default(5),
      topic: z
        .enum(["general", "news", "finance"])
        .可选()
        .default("general"),
      includeRawContent: z.boolean().可选().default(false),
    }),
  },
);
...
在实际使用中, most callers only encounter two slots: `USER` (your `system_prompt=`) and `BASE` (the SDK default). Selecting a model with a built-in profile—Anthropic or OpenAI today—adds a `SUFFIX`. The full four-part assembly is mainly relevant when you author a custom `HarnessProfile` or debug why a profile's text appears where it does.
...
默认情况下, Deep Agents have access to the following middleware:
...
The `deepagents` package also exposes [`createSummarizationMiddleware`](https://reference.langchain.com/javascript/deepagents/middleware/createSummarizationMiddleware) for the same workflow. 更多详情, see [Summarization](/oss/javascript/deepagents/context-engineering#summarization).
...
Tools for a deep agent can make use of virtual file systems to store, access, and edit files. 默认情况下, deep agents use a [`StateBackend`](https://reference.langchain.com/javascript/deepagents/backends/StateBackend).
...
Use a sandbox backend when you want your deep agent to write files, 安装依赖, and run commands without changing anything on your local machine.
...
你可以使用 [skills](/oss/javascript/deepagents/overview) to provide your deep agent with new capabilities and expertise.
...
例如 skills, see [Deep Agents example skills](https://github.com/langchain-ai/deepagentsjs/tree/main/examples/skills).
...
Use [`AGENTS.md` files](https://agents.md/) to provide extra context 给你的深度智能体。
