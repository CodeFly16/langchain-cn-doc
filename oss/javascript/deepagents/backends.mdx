...
深度智能体通过以下工具向智能体暴露文件系统接口： `ls`, `read_file`, `write_file`, `edit_file`, `glob`, and `grep`. 这些工具通过可插拔后端运行。 The `read_file` tool natively supports image files (`.png`, `.jpg`, `.jpeg`, `.gif`, `.webp`) across all backends, returning them as multimodal content blocks.
...
沙箱和 [`LocalShellBackend`](https://reference.langchain.com/javascript/deepagents/backends/LocalShellBackend) 还提供一个 `execute` tool.
本页面 explains how to:
...
以下是一些预构建的文件系统后端 你可以快速与深度智能体一起使用：

| 内置后端 | 描述 |
|---|---|
| [Default](#statebackend) | `agent = create_deep_agent(model="google_genai:gemini-3.1-pro-preview")` <br></br> 线程作用域. The default filesystem backend for an agent is stored in `langgraph` state. Files persist across turns within a thread (via your checkpointer) and are not shared across threads. |
| [Local filesystem persistence](#filesystembackend-local-disk) | `agent = create_deep_agent(model="google_genai:gemini-3.1-pro-preview", backend=FilesystemBackend(root_dir="/Users/nh/Desktop/"))` <br></br>This gives the deep agent access to your local machine's filesystem. You can specify the root directory that the agent has access to. 请注意 any provided `root_dir` must be an absolute path. |
...
| [Composite](#compositebackend-router) | 线程作用域 by default, `/memories/` persisted across threads. The Composite backend is maximally flexible. You can specify different routes in the filesystem to point towards different backends. See Composite routing below for a ready-to-paste example. |
...
****工作原理：****
- 将文件存储在 LangGraph 智能体状态中 for the current thread via [`StateBackend`](https://reference.langchain.com/javascript/deepagents/backends/StateBackend).
...
请注意 this backend is shared between the supervisor agent and subagents, and any files a subagent writes will remain in the LangGraph agent state
...
1. **Always** use `virtual_mode=True` with `root_dir` to enable path-based access restrictions (blocks `..`, `~`, and absolute paths outside root).
   请注意 the default (`virtual_mode=False`) provides no security even with `root_dir` set.
</Warning>
...
****工作原理：****
- 在可配置的目录下读写真实文件 `root_dir`.
- You can 可选ly set `virtual_mode=True` to sandbox and normalize paths under `root_dir`.
...
Notes:
- 更长的前缀优先匹配 (for example, route `"/memories/projects/"` can override `"/memories/"`).
...
- 路径是绝对的 (`/x/y.txt`). Decide how to map them to your storage keys/rows.
...
- All query methods (`ls`, `read`, `readRaw`, `grep`, `glob`) must return structured Result objects (e.g., `LsResult`, `ReadResult`) with an 可选 `error` field.
...
有关完整的 set of options including rule ordering, subagent permissions, and composite backend interactions, see the [permissions guide](/oss/javascript/deepagents/permissions).
...
- `FileInfo` with fields: `path` (required), 可选ly `is_dir`, `size`, `modified_at`.
...
- **`grep(pattern: string, path?: string | null, glob?: string | null) → GrepResult`**
  - 搜索文件内容 for a literal text pattern. Binary files (determined by MIME type) are skipped. On failure, return `{ error: "..." }`.
...
- **`FileInfo`** — `path` (required), 可选ly `is_dir`, `size`, `modified_at`.
