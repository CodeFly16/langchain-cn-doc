...
有关完整的 lifecycle table, async [graph factory](/langsmith/graph-rebuild) notes, TTL behavior, LangGraph Deployment wiring, and client-side examples, see [Sandbox lifecycle](/oss/javascript/deepagents/going-to-production#lifecycle) in Going to production.
...
你也可以 call the backend `execute()` method directly in your application code.
