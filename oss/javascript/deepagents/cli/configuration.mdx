...
All CLI-specific environment variables use a `DEEPAGENTS_CLI_` prefix (e.g., `DEEPAGENTS_CLI_AUTO_UPDATE`, `DEEPAGENTS_CLI_DEBUG`). 请参阅[CLI environment variable reference](#cli-environment-variable-reference)了解完整的list.
...
<Steps>
    <Step title="Get a key">
        Sign up at [tavily.com](https://tavily.com) and copy the key (it starts with `tvly-`). The 免费层级 is sufficient for most CLI usage.
    </Step>
...
<ResponseField name="models" type="string[]" post={["可选"]}>
    ...
    This key is 可选. You can always pass any model name directly to `/model` or `--model` regardless of whether it appears in the switcher; the provider validates the name at request time.
</ResponseField>

<ResponseField name="api_key_env" type="string" post={["可选"]}>
    The **name** of the environment variable that holds the API key (e.g., `"OPENAI_API_KEY"`), not the key itself. The CLI reads the credential from this env var at startup to verify access before creating the model. Most chat model packages read from a default env var automatically. 请参阅[Provider reference](/oss/javascript/deepagents/cli/providers#provider-reference) table for which variable each provider checks.
</ResponseField>

<ResponseField name="base_url" type="string" post={["可选"]}>
    Override the base URL used by the provider, if supported. Refer to your provider packages' [reference docs](https://reference.langchain.com/python/integrations/)了解更多info.
</ResponseField>
...
Some provider packages accept a `base_url` to override the default endpoint. 例如, `langchain-ollama` defaults to `http://localhost:11434` via the underlying `ollama` client. To point it elsewhere, set `base_url` in your configuration:
...
This is entirely 可选. You can always switch to any model by specifying its full name directly:
...
你可以使用 any [LangChain `BaseChatModel`](https://reference.langchain.com/python/langchain_core/language_models/#langchain_core.language_models.BaseChatModel) subclass using `class_path`. The CLI imports and instantiates the class directly — no built-in provider package required.
...
`api_key_env` and `base_url` are 可选. `class_path` providers are expected to handle their own authentication internally — useful when your model uses custom auth (JWT tokens, proprietary headers, mTLS, etc.) rather than a standard API key:
...
Your provider package may 可选ly provide model profiles at a `_PROFILES` dict in `<package>.data._profiles` in lieu of defining them under the `models` key. See LangChain [model profiles](https://github.com/langchain-ai/langchain/tree/master/libs/model-profiles)了解更多info.

---

## Skills extra allowed directories

默认情况下, when the CLI loads skills it validates that a resolved skill file path stays inside one of the standard [skill directories](/oss/javascript/deepagents/data-locations#skills). This prevents symlinks inside skill directories from reading arbitrary files outside those roots.
...
Define custom themes under `[themes.<name>]` sections in `config.toml`. Each section requires `label` (str). `dark` (bool) defaults to `false` if omitted — set to `true` for dark themes. All color fields are 可选 — omitted fields fall back to the built-in dark or light palette based on the `dark` flag.
...
你也可以 check for and install updates manually at any time with the `/update` slash command, which bypasses the cache and reports success or failure inline.
...
<ResponseField name="DEEPAGENTS_CLI_AUTO_UPDATE" type="string" post={["可选"]}>
    Enable automatic CLI updates (`1`, `true`, or `yes`).
</ResponseField>

<ResponseField name="DEEPAGENTS_CLI_DEBUG" type="string" post={["可选"]}>
...
<ResponseField name="DEEPAGENTS_CLI_DEBUG_FILE" type="string" default="/tmp/deepagents_debug.log" post={["可选"]}>
    Path for the debug log file.
</ResponseField>

<ResponseField name="DEEPAGENTS_CLI_EXTRA_SKILLS_DIRS" type="string" post={["可选"]}>
    Colon-separated paths added to the [skill containment allowlist](#skills-extra-allowed-directories).
</ResponseField>

<ResponseField name="DEEPAGENTS_CLI_LANGSMITH_PROJECT" type="string" post={["可选"]}>
...
<ResponseField name="DEEPAGENTS_CLI_NO_UPDATE_CHECK" type="string" post={["可选"]}>
    Disable automatic update checking when set.
</ResponseField>

<ResponseField name="DEEPAGENTS_CLI_SHELL_ALLOW_LIST" type="string" post={["可选"]}>
    Comma-separated shell commands to allow (or `recommended` / `all`).
</ResponseField>

<ResponseField name="DEEPAGENTS_CLI_USER_ID" type="string" post={["可选"]}>
