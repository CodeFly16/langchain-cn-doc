...
深度智能体 CLI 是一个基于 [深度智能体 SDK](/oss/javascript/deepagents/quickstart).
The CLI works with any LLM 支持工具调用的 并允许你在输入之间切换 LLM。
它保留从对话中学习的持久记忆, 跨会话维护上下文, 使用可自定义的技能, 并通过审批控制执行代码。
...
<Steps>
    ...
    <Step title="Install and launch" icon="terminal">
        OpenAI, Anthropic, and Google are installed by default. Other providers (Ollama, Groq, xAI, etc.) are available as 可选 extras—see [Providers](/oss/javascript/deepagents/cli/providers)了解详情.
    ...
    <Step title="Add provider credentials" icon="key">
        The CLI works with any LLM 支持工具调用的. Use the `/auth` command to set an API key for your chosen providers — see [Provider credentials](/oss/javascript/deepagents/cli/configuration#provider-credentials)了解完整的flow and storage details.
    ...
    <Step title="Enable tracing (可选)" icon="chart-dots">
        ...
        LANGSMITH_PROJECT=可选-project-name  # Specify a project name or default to "deepagents-cli"
        ```

        更多详情s and usage, see [Trace with LangSmith](#trace-with-langsmith).
    </Step>
</Steps>
...
<Accordion title="Full list of built-in tools">
    ...
    | Tool | 描述 | Human-in-the-Loop |
    |------|-------------|-------------------|
    | `ls` | List files and directories | - |
    | `read_file` | Read contents of a file; multimodal content for select models | - |
    | `write_file` | Create or overwrite a file | 必需<sup>1</sup> |
    | `edit_file` | Make targeted edits to existing files | 必需<sup>1</sup> |
    | `glob` | Find files matching a pattern | - |
    | `grep` | Search for text patterns across files | - |
    | `execute` | Execute shell commands locally or in a remote sandbox | 必需<sup>1</sup> |
    | `web_search` | Search the web using Tavily (requires `TAVILY_API_KEY` — see [Enable web search](/oss/javascript/deepagents/cli/configuration#enable-web-search-with-tavily)) | 必需<sup>1</sup> |
    | `fetch_url` | Fetch and convert web pages to markdown | 必需<sup>1</sup> |
    | `task` | Delegate work to subagents for parallel execution | 必需<sup>1</sup> |
...
<AccordionGroup>
    ...
    <Accordion title="CLI commands" icon="terminal">
        ...
        All management subcommands support `--json` for machine-readable output. See [command-line options](#command-line-options)了解详情.
...
有关完整的 reference — including `config.toml` schema, provider parameters, profile overrides, and hook configuration — see [Configuration](/oss/javascript/deepagents/cli/configuration).
...
<AccordionGroup>
    <Accordion title="Slash commands" icon="slash">
        ...
        - `/agents` - Hot-swap between pre-configured agents without relaunching. See [Command reference](/oss/javascript/deepagents/cli/overview#command-reference)了解详情
        - `/auth` - Manage stored API keys for model providers. See [Provider credentials](/oss/javascript/deepagents/cli/configuration#provider-credentials)了解详情
...
你也可以 pipe input via stdin. When input is piped, the CLI automatically runs non-interactively:
...
To override for a specific project, add the same keys to a `.env` in the project directory. See [environment variables](/oss/javascript/deepagents/cli/configuration#environment-variables)了解完整的loading order.

你也可以 set these as shell environment variables if you prefer. Shell exports always take precedence over `.env` values, so this is a good option for temporary overrides or testing:
...
<Accordion title="Separate agent traces from app traces">
    When invoking the CLI programmatically from a LangChain application (e.g., as a subprocess in [non-interactive mode](#non-interactive-mode-and-piping)), both your app and the CLI produce LangSmith traces. 默认情况下, these all land in the same project.
    ...
    你也可以 scope LangSmith credentials to the CLI using the [`DEEPAGENTS_CLI_` prefix](/oss/javascript/deepagents/cli/configuration#deepagents_cli_-prefix) (e.g., `DEEPAGENTS_CLI_LANGSMITH_API_KEY`).
</Accordion>

When configured, the CLI displays a status line with a link to the LangSmith project. In supported terminals, click the link to open it directly. 你也可以 use `/trace` to print the URL and open it in your browser.
