...
The Deep Agents CLI supports any [chat model provider compatible with LangChain](/oss/javascript/integrations/chat), unlocking use for virtually any LLM 支持工具调用的. Any service that exposes an OpenAI-compatible or Anthropic-compatible API also works out of the box—see [Compatible APIs](/oss/javascript/deepagents/cli/configuration#compatible-apis).
...
1. **Install provider packages**
    ...
    Each model provider requires installing its corresponding LangChain integration package. These are available as 可选 extras when installing the CLI, done intentionally to keep the application lightweight:
...
2. **Set credentials**
    ...
    <Tabs>
        ...
        <Tab title="Other">
            The CLI works with any LLM 支持工具调用的. See [Provider reference](#provider-reference)了解完整的list of supported providers and their required environment variables.
    ...
    你也可以 scope credentials to the CLI with the [`DEEPAGENTS_CLI_` prefix](/oss/javascript/deepagents/cli/configuration#deepagents_cli_-prefix).
...
1. **Use the interactive model switcher** with the `/model` command. This displays available models sourced from each installed LangChain provider package's [model profiles](/oss/javascript/langchain/models#model-profiles).
    ...
    <Note>
        Not all models appear here. If yours is missing, pass the model name directly (e.g. `/model gpt-5.5`). See [Which models appear in the switcher](#which-models-appear-in-the-switcher)了解详情.
    </Note>
2. **Specify a model name directly** as an argument, e.g. `/model gpt-5.5`. 你可以使用 any model supported by the chosen provider, regardless of whether it appears in the list from option 1. The model name will be passed to the API request.
...
The `/model` selector dynamically builds its list from installed provider packages. Expand below了解完整的criteria and troubleshooting.
...
<Accordion title="How the switcher builds its model list" icon="list-search">
    ...
    | --- | --- | --- |
    | Entire provider missing from switcher | Provider package not installed | 安装 package (e.g. `uv tool install deepagents-cli --with langchain-groq`) |
...
2. **Mid-session via `/model --model-params`.** Same JSON syntax—swaps params (and 可选ly the model) without restarting:
...
3. **Persistent in `config.toml`.** Provider-level defaults (with 可选 per-model sub-tables) that apply on every launch:
