...
Allow the agent to read memory files but prevent it from modifying them. 这在以下情况很有用 for organization-wide policies or shared knowledge bases that should only be updated by application code. See [read-only vs writable memory](/oss/javascript/deepagents/memory#read-only-vs-writable-memory)了解更多context.
