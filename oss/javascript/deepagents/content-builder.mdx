...
本指南 demonstrates how to build a content writing agent from scratch using [Deep Agents](/oss/javascript/deepagents).

你构建的智能体将：

1. 从 `AGENTS.md` 和技能文件夹加载语音和工作流规则
1. 将网络研究委派给专门的子智能体 with `web_search`
1. 按照加载的技能草拟博客或社交内容
1. 生成封面或社交图片 with Gemini and save files under the project directory

本教程中的代码 wires in image generation tools and a filesystem backend so the agent can read and write posts, research notes, and images under the project directory. 有关完整的 runnable project, see the [content-builder-agent](https://github.com/langchain-ai/deepagents/tree/main/examples/content-builder-agent) example.
...
Create `content_writer.ts` in the project root. 以下 sections belong in one file, in order.
...
浏览完整的 [content-builder-agent example](https://github.com/langchain-ai/deepagents/tree/main/examples/content-builder-agent) 在 GitHub 上, including the Rich-based streaming UI.
