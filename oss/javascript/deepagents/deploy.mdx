...
`deepagents deploy` 获取你的智能体配置和文件，并将它们一起部署为 [LangSmith Deployment](/langsmith/deployment).

LangSmith 部署是水平可扩展的服务器 包含 30 多个端点，包括 MCP、A2A、Agent Protocol, 人机协作和记忆 API。 基于开放标准构建：
...
<Warning>
    Deep Agents Deploy is currently in beta and requires `deepagents-cli>=0.0.36`. APIs, configuration format, and behavior may change between releases. 请参阅[releases page](https://github.com/langchain-ai/deepagents/releases) for detailed changelogs.
</Warning>
...
默认情况下, `deepagents deploy` looks for `deepagents.toml` in the current directory. Pass `--config` to use a different path:
...
| File | Purpose |
| --- | --- |
| `deepagents.toml` | Agent config — name, model, 可选 sandbox |
...
| File | 描述 |
| ---- | ----------- |
| **`deepagents.toml`** | 必需. Configures the agent's [identity](#configuration-file), [authentication](#auth), [sandbox environment](#sandbox), and the deployment of a prebuilt [frontend](#frontend) Reach chat UI. |
| **`AGENTS.md`** | 必需. [Memory](/oss/javascript/deepagents/memory) for the agent. Provides persistent context (project conventions, instructions, preferences) that is always loaded at startup. Read-only at runtime. |
...
| **`mcp.json`** | _Optional_. MCP tools (HTTP/SSE). If `mcp.json` exists, it is included in the deployment and [`langchain-mcp-adapters`](https://pypi.org/project/langchain-mcp-adapters/) is added as a dependency. See [MCP (LangChain)](/oss/javascript/langchain/mcp)了解更多information. <Warning> `mcp.json` must only contain servers using `http` or `sse` transports. Servers using `stdio` transport are not supported in deployed environments because there is no local process to spawn. <br /><br /> Convert stdio servers to HTTP or SSE before deploying. </Warning> |
| **`subagents/`** | _Optional_. Specialized [Subagents](#subagents) the main agent can delegate to. Each subdirectory must contain a `deepagents.toml`, `AGENTS.md`, and 可选ly a `skills` folder. Auto-discovered at bundle time. |
...
`deepagents.toml` configures the agent's identity and sandbox environment. Only the `[agent]` section is required. The `[sandbox]` section is 可选 and defaults to no sandbox.

| Field | Type | 描述 |
| --- | --- | --- |
| [`[agent]`](#agent) | Object | 必需. Core agent identity (name and model). |
...
| [`[auth]`](#auth) | Object | _Optional_. Configure authentication on the deployed agent. 必需 when `[frontend].enabled = true`. |
...
| Field | Type | 描述 |
| --- | --- | --- |
| `name` | `string` | 必需. Name for the deployed agent. Used as the assistant identifier in LangSmith. |
...
Add an `[auth]` section to configure authentication on the deployed agent. `[auth]` is **required** when `[frontend].enabled = true`; otherwise it is 可选 (without it, LangSmith Deployment's default `x-api-key` requirement applies).
...
| Field | Type | 描述 |
| --- | --- | --- |
| `provider` | `string` | 必需. Auth provider. Supported values: `"supabase"`, `"clerk"`, `"anonymous"`. |
...
Place a `.env` file alongside `deepagents.toml` with your API 密钥：

```bash
# 必需 — model provider keys
ANTHROPIC_API_KEY=sk-...
OPENAI_API_KEY=sk-...
# ...etc.

# 必需 for deploy and LangSmith sandbox
...
# 必需 if [auth] provider = "supabase"
SUPABASE_URL=https://your-project.supabase.co
SUPABASE_PUBLISHABLE_DEFAULT_KEY=eyJhbGc...

# 必需 if [auth] provider = "clerk"
CLERK_SECRET_KEY=sk_test_...

# 必需 if [frontend].enabled = true AND [auth] provider = "clerk"
...
- **`[auth] provider = "anonymous"`** — the bundler ships a permissive auth handler. The API is open to anyone with the deploy URL. No header required. (必需 when shipping `[frontend]` without real per-user auth.)
...
```txt
user/
└── AGENTS.md          # 可选 — seeded as empty if not provided
```
...
Subagents let the main agent delegate specialized tasks to isolated child agents. Each subagent has its own system prompt, 可选 skills, and 可选 MCP tools. The main agent receives a `task` tool that dispatches work to subagents by name.
...
└── subagents/
    ├── researcher/
    │   ├── deepagents.toml       # name, description, 可选 model override
    │   ├── AGENTS.md             # subagent system prompt
    │   ├── skills/               # 可选 — subagent-specific skills
    │   │   └── analyze-market/
    │   │       └── SKILL.md
    │   └── mcp.json              # 可选 — HTTP/SSE MCP tools
    └── writer/
        ├── deepagents.toml
        └── AGENTS.md
```
...
| Field | Type | 描述 |
| --- | --- | --- |
| `name` | `string` | 必需. Unique identifier for the subagent. Must be unique across all subagents. |
| `description` | `string` | 必需. What this subagent does. The main agent uses this to decide when to delegate. Must be non-empty. |
